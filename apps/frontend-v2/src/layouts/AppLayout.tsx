...
function getSelectedKey(path: string): string {
  ...
  if (['/import', '/email', '/audit', '/schema', '/config', '/users', '/op-log', '/backup', '/db-migration'].includes(path)) return path;
  return '/';
}
...
function getOpenKeysForPath(path: string): string[] {
  ...
  if (['/import', '/email', '/audit', '/schema', '/config', '/users', '/op-log', '/backup', '/db-migration'].includes(path)) return ['system'];
  return [];
}
