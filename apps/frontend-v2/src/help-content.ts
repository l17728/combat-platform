...
## v3.5.0 — 2026-06-08 (责任图谱可视化重做)

完全重做责任图谱,从不可读的毛线球图改为分层聚合视图。

### 📊 可视化重做

- **概览图** — 按升级规则和人员聚合,节点控制在 20-30 个,Mermaid 可读
- **人员负载表** — Tab 页展示 Top 15 人员的分配数/上报数/冲突数,支持排序
- **冲突关系表** — 去重后展示 Top 20 冲突对,不再画 1600+ 条虚线
- **升级规则表** — 独立 Tab 展示所有升级规则的 SLA / 角色 / 关联任务数
- **统计卡片** — 顶部 4 个统计卡片: 任务总数 / 人员总数 / 冲突组数 / 升级规则数
- **后端聚合** — 冲突边去重(双向→单向),人员按负载排序取 Top 15
...
- **管理模板按钮** — 攻关单详情 → 求助网络 Tab 新增"管理模板"按钮
- **模板删除** — 弹出模板列表 Modal，支持一键删除不需要的支撑模板
- 确认对话框防止误删

### 🔗 手动关系管理（P1）

- **手动关联** — 关联全景页新增"手动关联"按钮，可在任意两个节点之间创建关联
- **搜索目标节点** — 输入名称实时搜索，选择目标后填写关联原因
- **手动关联列表** — 紫色虚线区域展示当前节点的所有手动关联，支持删除
- 与 AI 自动提议关系互补——手动关系即时生效，无需等待 AI 发现

### 📌 UI 置顶 / 快捷访问（P1）

- **AI 回答置顶** — Hermes AI 助手回答附带 uiSpec 时，气泡下方出现"📌 置顶到首页"按钮
- **快捷访问区域** — Dashboard 首页顶部新增"快捷访问"卡片，展示所有置顶的视图
- **取消置顶** — 每个置顶卡片右上角可一键取消

### ⚡ 自定义命令（P2）

- **新页面** — 系统管理 → 自定义命令，完整的 CRUD 管理界面
- **创建命令** — 输入名称、模板（含 {参数名} 占位符），系统自动检测参数
- **执行命令** — 动态生成参数输入表单，提交后展示解析结果和 API 请求
- **删除命令** — 确认后删除

### 🕸️ 责任图谱可视化（P2）

- **新页面** — 系统管理 → 责任图谱，Mermaid 流程图渲染
- **三类关系** — 升级链（事件级别 → SLA → 角色）、人员分配、冲突关系
- **统计概览** — 页面顶部显示节点数和关系数
- **全屏模式** — 一键放大查看完整图谱
...
- **备份恢复页增强** — 新增"异地备份"按钮
- **SSH 配置表单** — 填写目标主机、用户、端口、远程目录、密钥/密码
- **Dry Run 模式** — 开关控制，仅测试连接不实际传输
- **执行结果展示** — 成功显示 JSON 摘要，失败显示错误日志

### 🔓 权限调整

- **合规追溯卡片** — 攻关单侧边栏的"合规追溯"卡片对所有用户可见（此前仅 leader 可见）
...
- **前端路由 Guard** — 11 个系统管理路由添加 AdminGuard，normal user 无法直接访问系统管理页面
...
- 系统管理功能（设置、Schema、导入、升级、备份等）仅管理员可操作
- Guest 游客可浏览所有页面（含系统管理），但不能执行任何写操作
- 前端路由保护 + 后端 API 权限检查双重验证，任一层被绕过均有另一层兜底

## v3.0.1 — 2026-06-04 (Guest 只读防护 + 安全加固)

- Guest 游客三层只读防护（后端中间件 + 前端 API 拦截器 + 组件级 guard）
- SuperAdmin 自动提升
- 品牌更名「会战管理」
- 登录页安全加固（移除默认密码提示）

## v3.0.0 — 2026-06-03 (共享功能 + SaaS 多租户)

本版是平台级大版本，新增统一的共享机制和完整的 SaaS 多租户架构。
...
- **访问趋势图**: 分享链接最近 30 天每日访问量柱状图
- **文章复制到攻关单**: 将知识库文章一键复制到目标攻关单的局部知识库
- **公共分享页面**: \`/s/:token\` 独立页面，无需登录即可查看分享内容；密码保护文章弹出密码输入框
- **安全**: 加锁文章禁止分享；攻关单分享脱敏；token 不可预测不可枚举

### ② SaaS 多租户 (Phase 0-4)

- **多租户架构**: \`SAAS_MODE=1\` 启用多租户，不设则单租户模式，向后兼容
- **平台管理**: superAdmin 管理租户 CRUD + 暂停/恢复 + 资源统计
- **租户详情**: 用户列表 + 资源用量 + 配额进度条 + 租户设置
- **注册流程改造**: 登录页双 Tab（登录 + 注册），注册支持「创建团队」和「加入团队」
- **Guest 免登录体验**: 自动创建临时用户，1 天 JWT，24h cron 定期清理
- **计划配额**: free/pro/enterprise 三档（用户数 + 节点数），创建用户时自动检查

### ③ 新增页面

| 页面 | 路由 | 说明 |
|------|------|------|
| 公共分享页 | \`/s/:token\` | 无需登录查看分享内容 |
| 平台管理 | \`/platform\` | 租户列表（superAdmin） |
| 租户详情 | \`/platform/tenants/:id\` | 用户/用量/配额（superAdmin） |
...
## v2.9.0 — 2026-06-02 (知识库大版本 + Markdown 全新渲染引擎)

本版是知识库的重大升级：点赞/加锁/审计 + Markdown 支持 Mermaid 图和视频嵌入。

### ① 知识库 Wiki 大版本更新

- **点赞系统**: 文章可点赞/取消点赞，列表按三级排序（加锁 → 点赞 → 普通），同级别内可拖拽调序
- **加锁/密码保护**: 文章可加锁并设置密码，删除加锁文章需输入密码验证
- **编辑权限**: 加锁文章仅创建者或管理员可编辑，其他用户看不到编辑按钮
- **删除权限**: 仅创建者或管理员可删除；删除后自动聚焦到相邻文章
- **审计日志**: 创建/编辑/删除/点赞操作自动写入审计日志，可在审计日志页面查看
- **折叠模式**: 点击左箭头可将目录折叠为彩色圆圈首字（类似头像），点击圆圈选中文章

### ② MarkdownRenderer 全新渲染引擎

- **Mermaid 图**: \`\`\`mermaid 代码块自动渲染为 SVG（流程图、时序图、甘特图等）
- **YouTube 视频**: YouTube 链接渲染为缩略图+播放按钮，点击新窗口播放
- **Bilibili 嵌入**: \`<iframe src="player.bilibili.com/...">\` 渲染为响应式播放器，\`bilibili.com/video/BVxxx\` 自动转换为嵌入 URL
- **HTML video**: \`<video controls><source src="..."></video>\` 直链视频内嵌播放
- **安全**: rehype-raw + rehype-sanitize 白名单，仅允许 video/source/iframe，iframe 域名限制 YouTube/Bilibili

### ③ 知识图谱边类型重命名

- REF → 分配, CONFLICTS_WITH → 冲突, ANCHORED_TO → 关联, ESCALATED_TO → 上报, RELATES_TO → 处理, OVERLAPS_WITH → 重叠
- 影响约 40 个文件 + 生产数据库 1,187 条边
...
## v2.8.0 — 2026-06-01 (知识图谱可视化 + 边类型中文重命名)

- 知识图谱边按类型着色（6 种颜色），底部图例栏
- 边 hover 弹出类型 tooltip，隐藏边文字标签减少视觉干扰
- 节点类型筛选下拉框
- 6 种边类型全部重命名为中文名称
- Hermes AI 文字驱动 UI 操作 + 写工具启用
- Weink AI 四级 fallback 机制

## v2.7.0 — 2026-05-30 (知识图谱冲突检测 + 攻关单详情增强)

- 知识图谱冲突与重叠关系自动检测
- 攻关单详情页增强
- 进展日志优化

## v2.6.0 — 2026-05-28 (HermesChat AI 助手 + 知识库基础功能)

- HermesChat AI 助手集成，支持多轮对话
- 知识库基础功能：Markdown 编辑、搜索、文章管理
- 会话记忆和上下文保持

## v2.5.0 — 2026-05-25 (仪表盘大屏 + 文档上传)

- 作战态势仪表盘大屏：KPI 卡片、状态分布、最近活跃
- 文档上传管理
- 文件预览和下载

## v2.4.0 — 2026-05-22 (权限系统 + 稳定性)

- 用户角色权限体系优化（admin/leader/normal）
- 操作日志和审计增强
- 多项 Bug 修复和性能优化

## v2.3.10 — 2026-06-01 (知识库 + API文档 + 前端代码拆分)

本版新增三大特性: 全局/局部知识库 Wiki、API 自动文档（Swagger UI）、前端路由级代码拆分（首屏减重）。

### ① 知识库 Wiki

- **全局知识库**：态势首页新增「知识库」tab（信息广场后面），存放全局知识、文档、规范
- **局部知识库**：攻关详情页自定义 tab 新增「知识库」类型，为单个攻关单建立专属知识库
- Markdown 编辑 + 实时渲染（支持 GFM 表格、代码块等）
- 文章列表 + 搜索 + 新建 + 编辑 + 删除
- 自动记录创建人和更新时间
...
- 30+ 页面从静态 import 改为 React.lazy 路由级懒加载
- 3.5MB 单文件 → 按需加载，首屏只加载当前路由所需代码
- Suspense 统一 loading 态，无白屏闪烁
- 预期首屏 bundle 减重 60-80%

## v2.3.9 — 2026-06-01 (邮件增强 + 邀请管理 + 运营大屏)

本版新增三大特性: 邮件摘要支持 HTML 模板和自定义时间段、管理员邀请成员加入并分配角色、全屏运营态势大屏。

### ① 邮件增强

- 邮件摘要从纯文本升级为 **HTML 模板**（渐变头部 + 表格 + 统计卡片 + 样式化邮件）
- 同时保留纯文本版本（不支持 HTML 的客户端自动降级）
- 支持**自定义时间段**：预览和发送时可选择任意天数（1-90天），不限于每天/每周
- MailSender 接口扩展支持 html 和 attachments

### ② 邀请管理

- 系统管理 → 邀请管理（仅管理员）
- 管理员输入邮箱、选择角色（管理员/负责人/成员），系统自动发送邀请邮件
- 邀请邮件包含 **HTML 样式化链接** 和邀请码
- 被邀请人点击链接进入注册页，自动获得预设角色
- 邀请码 7 天有效（可自定义），过期自动失效
- 支持复制邀请链接、删除未使用邀请
- 已使用的邀请标记为"已使用"，不可重复使用

### ③ 运营大屏

- 工具 → 运营大屏（所有用户可访问）
- **深色主题**全屏数据可视化面板
- 四大 KPI 卡片：攻关单总数、进行中、已解决、贡献总数
- 状态分布条形图（实时比例）
- 最近活跃列表（标题 + 状态标签 + 时间）
- **全屏模式**：点击全屏按钮进入投屏模式
- **自动刷新**：每 30 秒自动拉取最新数据
...
## v2.3.8 — 2026-06-01 (Webhook 事件订阅 + 邮件摘要 + 表单内联字段)

本版新增三大能力: Webhook 事件推送实现系统联动、邮件摘要定时汇总动态、攻关详情页内联添加字段与 SchemaWizard 联动。

### ① Webhook 事件订阅

- 系统管理 → Webhook 订阅（仅管理员）
- 支持 11 种事件：节点创建/更新/删除、状态流转、进展追加、求助/问题反馈/提醒/升级/用户创建/系统升级
- 新建订阅时选择回调 URL + 事件类型
- 每个订阅独立密钥（X-Webhook-Secret），用于验签
- 支持启用/禁用开关、测试推送、编辑、删除
- 事件触发后异步 HTTP POST，10s 超时，不阻塞主流程

### ② 邮件摘要

- 系统管理 → 邮件摘要
- 支持每天/每周自动汇总：新建攻关单、状态流转、新增贡献、总体统计
- 可配置收件人列表、包含内容开关
- 手动预览当前周期摘要 + 手动发送
- 后台每小时检查，到时间自动发送（需先在「邮件设置」配置 SMTP）

### ③ 攻关详情内联字段添加

- 攻关详情 → 基础信息 tab → 「添加字段」按钮
- 直接在详情页添加新字段，无需跳转 SchemaWizard
- 支持字段类型选择（文本/长文本/数字/日期/枚举/布尔）
- 枚举字段支持两种模式：手动输入枚举值 或 绑定配置中心键名
- 添加后字段立即出现在当前表单，与 SchemaWizard 双向同步
- 字段管理 Popover 内也增加了「添加新字段」快捷入口

## v2.3.7 — 2026-06-01 (UX 中期 II: 暗黑模式 + 产品引导 + 看板配置)

本版聚焦用户体验提升:暗黑模式护眼、首次登录引导降低上手门槛、Dashboard 看板可个性化配置。

### ① 暗黑模式

- 顶部栏新增 🌙/☀️ 切换按钮,一键切换亮色/暗色主题
- 用户偏好自动保存到浏览器,刷新后保持
- 全站浮动面板(HermesChat / NotificationBell / DynamicCustomTab)背景色跟随主题
- Markdown 渲染(markdown.css)暗黑模式适配:代码块、表格、引用等
- 基于 Ant Design 5 \`darkAlgorithm\`,所有组件自动适配

### ② 产品 Tour 引导

- 首次登录自动触发 **Dashboard Tour**(6 步):概览→侧边栏→统计→我的任务→AI 助手→命令面板
- 首次进入攻关列表触发 **AttackList Tour**(4 步):筛选→新建→导出→批量操作
- 管理员额外触发 **Admin Tour**(4 步):配置中心→Schema→用户管理→系统升级
- 顶部栏用户菜单「重播引导」可重置所有 Tour

### ③ Dashboard 看板配置

- 右上角 ⚙️ 设置按钮打开看板配置面板
- 可勾选显示/隐藏 6 个卡片:统计概览 / 分配给我 / 我的关注 / SLA 风险 / 最近活跃 / 状态分布
- 上移/下移按钮调整卡片顺序
- 恢复默认一键重置
- 偏好保存到浏览器,跨会话保持

## v2.3.6 — 2026-06-01 (Hermes 高级能力: 写工具 + 会话记忆)
...
## v2.3.5 — 2026-06-01 (三桶 — Hermes 体验收尾 + Schema-as-UI 全栈化 + 多视图)

本版聚焦"把 v2.3.4 几条主线完整跑通":Hermes 体验收尾(动态 model 列表 + golden set 15/15 + 部署 drop-in 自愈)、Schema-as-UI 推到全部 7 个 nodeType、多视图(Kanban/Calendar/Pivot)首次落地。
...
- **/api/llm-settings/test 修 env-fallback**: admin 未存 DB 也能走 env 实测连接,补 v2.3.4 漏洞
- **Hermes prompt 优化**: 审计追溯类问题("admin 改过哪些"/"张三干过什么")LLM 现在会优先 \`get_audit(actor='X')\` 而非让用户澄清;golden set **15/15 通过**(v2.3.4 是 14/15)
- **deploy-direct.mjs 自愈 systemd drop-in**: 自动检测多个 drop-in 设同一 env key 时备份旧的、保留新的(吃下 v2.3.4 \`hermes.conf\` 累加坑);\`--keep-old-drop-ins\` flag 关闭自动清理
- 默认 model **glm-4-flash**(智谱免费层,零成本可用,首次部署即工作)

**② Schema-as-UI 全栈化**
- **v2.3.4 的 attackTicket 模式推到全部 7 个 nodeType**: contribution / teamContribution / person / helpRequest / bugReport / proposal / reminder
...
- 攻关 v2.3.2 React #310 Hooks 修复仍保留
...
> v2.3.4 完成的是"用户可见的中期 UX 槽位填上"(Inbox/面包屑/LLM UI);v2.3.5 完成的是把这些主线**贯穿落地** — Schema-as-UI 从 1 个 nodeType 到 7 个 + 视图从 1 种到 4 种 + LLM 从 14/15 到 15/15 + 部署从手工到自愈。这是把"组件化能力"转向"产品成熟度"的关键一版。配合 v2.3.4 的 LLM 直连 + UI 配置,**v2.3.5 之后管理员的全部日常运维都在浏览器里完成**:加字段、改 LLM、看通知、切视图,**无需 SSH、无需重启**。

## v2.3.4 — 2026-05-31 (四桶 — LLM 端到端 + Inbox + 面包屑 + Schema-as-UI)
...
**④ 文档 + 集成 Stage E**
- 顶部 release notes(本节)+ 每个受影响 page 章节内追加 v2.3.4 说明
- docs/V2.6_RELEASE.md / LLM_SETTINGS.md / NOTIFICATIONS.md / SCHEMA_AS_UI.md
- merge master → tag v2.3.4 → deploy 124.156.193.122 → 现网 verify(LLM 真跑 + Inbox + Schema 渲染)
...
> v2.3.3 把 hermes 从硬编码 intent 路由升级为 tool-using agent,但当时 LLM 仍走 opencode 中转,导致(a)现网部署要装 opencode、(b)配置改了要 SSH、(c)v2.3.3 现网 LLM ask 卡 120s 超时部分原因在 opencode SDK 适配链。v2.3.4 直接绕开:backend 用纯 fetch 打 OpenAI 兼容 endpoint,配置入 DB 由 UI 管。一旦做完这步,**v2.3.4 系列的运维成本接近零**——admin 在浏览器改 model/key/baseURL 保存即生效,key rotation 不重启。这是把"AI 能力是后端 SDK"转向"AI 能力是配置数据"的关键一步。

## v2.3.3 — 2026-05-31 (旗舰特性:Hermes Tool-using Agent + 14 通用工具)

本版把 Hermes 从「intent 路由器」升级为「Tool-using Agent」——LLM 自己看 schema 选工具,不再硬编码 intent 正则。v2.3.1 现网"有多少员工"这类问题(intent 命不中 → fallback-search 空)v2.3.3 直接命中 \`count_nodes\` 返回精确答案。
...
- 优雅降级: 旧 response 无 trace 时完全不渲染,与 v2.3.2 行为一致
...
- Backend **649/649** 通过(90 文件 = v2.3.1 baseline 575 + tools 57 + agent 17)
- Shared **28/28** 通过
...
> 把硬编码 intent 升级为通用工具集后,新需求(新报表、新指标、新过滤)不再要求"加一个 endpoint + 加一个 intent 正则";LLM 自己组合工具回答。代价是延迟变高(多轮 LLM + 工具往返),所以 \`auto\` 模式保留 intent 快路径处理高频简单问句。这是 v2.3.4+ Agent 能力进一步扩张的基座(memory / planner / reflection)。

## v2.3.2 — 2026-05-31 (Hot-fix)
...
## v2.3.1 — 2026-05-31 (三桶 — 现网加固 + 架构韧性 + 升级 UI 真实化)
...
- **MVP 限制**:本地上传包(不支持 GitHub release 拉取/PGP 签名校验,留 v2.3.1);自我升级生产化需在测试环境真跑 systemd 重启一次再上线
...
- **知识库**：全局知识文档库（技术规范/操作手册/团队文档），支持 Markdown 编辑（含 Mermaid 图、Bilibili/YouTube 视频嵌入）、搜索、点赞、加锁保护、拖拽排序、折叠目录；所有操作自动记录审计日志
...
### v2.3.5 — 详情页/抽屉全部 schema 驱动 + 透视视图
...
### v2.3.5 — 详情抽屉 schema 驱动
- 提案详情抽屉的字段、分组、只读规则全部来自 \`proposal\` schema(virtual:true,数据存专表但 UI 走 schema)
- 在 SchemaWizard 给 proposal 加字段(如审批意见、关联工单),抽屉立即出现该字段,无需改前端代码`,
  },
...
### v2.3.5 — 详情抽屉 schema 驱动
- 跟催提醒详情抽屉全部由 \`reminder\` schema 驱动(virtual:true);加字段在 SchemaWizard 操作,UI 自动响应`,
  },
...
4. 点击「发送」测试邮件是否正常`,
  ...
  webhookSettings: {
    title: "Webhook 订阅 - 使用帮助",
    content: `## Webhook 事件订阅

### 功能概述
将系统内的事件以 HTTP POST 方式实时推送到指定 URL，实现与外部系统联动。仅管理员可管理订阅。
...
### 推送格式
POST 请求体为 JSON，包含 \`event\`、\`timestamp\`、\`data\` 字段。请求头包含 \`X-Webhook-Secret\`（验签密钥）和 \`X-Webhook-Event\`（事件类型）。

### 操作
1. 点击「新建订阅」，填写回调 URL 和要订阅的事件
2. 保存后系统自动在对应事件触发时推送
3. 点击「测试」发送一条测试事件验证连通性
4. 可随时启用/禁用或删除订阅`,
  },
  digestSettings: {
    title: "邮件摘要 - 使用帮助",
...
### 功能概述
定时汇总系统动态（新建攻关单、状态流转、新增贡献、统计概览），发送到指定邮箱。

### 前提
需要先在「邮件设置」中配置 SMTP，否则无法发送。

### 配置项
- **频率**：每天 / 每周
- **收件人**：支持多个邮箱
- **内容开关**：总体统计 / 新建攻关单 / 状态流转 / 新增贡献
- **自定义天数**：预览和发送时可选择任意天数（1-90天）

### 邮件格式
- HTML 样式化邮件（渐变头部 + 表格 + 统计卡片）
- 同时保留纯文本版本（不支持 HTML 的客户端自动降级）

### 操作
1. 开启开关，设置频率和收件人
2. 设置天数，点击「预览」查看对应周期的摘要内容
3. 点击「立即发送」手动触发一次
4. 系统也会在配置的时间自动发送`,
  },
  invitation: {
...
### 功能概述
管理员通过邮件邀请成员加入平台，可预设角色（管理员/负责人/成员）。仅管理员可管理邀请。

### 流程
1. 管理员在「邀请管理」页面创建邀请：填写邮箱、选择角色
2. 系统自动发送邀请邮件到指定邮箱
3. 被邀请人点击邮件中的链接，进入注册页面
4. 注册成功后自动获得预设角色

### 邀请码规则
- 每个邀请码 12 位大写字母+数字
- 默认 7 天有效期（可自定义 1-90 天）
- 已使用的邀请码标记为"已使用"，不可重复使用
- 过期的邀请码自动失效
...
### 功能概述
全屏数据可视化面板，实时展示系统运营态势。适用于投屏监控、会议展示。

### 数据内容
- 四大 KPI：攻关单总数、进行中、已解决、贡献总数
- 状态分布条形图（按比例实时展示）
- 最近活跃列表（最新 10 条动态）
- 底部显示冲突统计
...
### 功能概述
知识库用于存放全局知识文档、技术规范、操作手册等结构化内容。支持 Markdown 格式（含 Mermaid 图、视频嵌入），可搜索、编辑、点赞、加锁。

### 两种知识库

#### 全局知识库（态势首页）
- 态势首页 → 知识库 tab（信息广场后面）
- 所有登录用户可查看，leader/admin 可编辑
- 适合存放：技术规范、通用操作手册、团队文档
...
| 操作 | 说明 |
|------|------|
| 新建 | 点击「新建」创建文章，输入标题和 Markdown 正文 |
| 查看 | 左侧列表点击文章查看内容，右侧展示渲染结果 |
| 编辑 | 点击编辑按钮修改标题或正文 |
| 删除 | 仅创建者或管理员可删除，删除后自动聚焦相邻文章 |
| 搜索 | 搜索框输入关键词快速过滤文章 |
| 点赞 | 点击👍按钮点赞/取消点赞，文章按点赞数排序 |
| 加锁 | 创建或编辑时可加锁并设置密码，加锁文章仅创建者/管理员可编辑 |
| 拖拽排序 | 同级别内（加锁/点赞/普通）可拖拽调整顺序 |
| 折叠 | 点击左箭头折叠目录，显示彩色圆圈首字；点击右箭头展开 |

### Markdown 高级功能

#### Mermaid 图
用 \`\`\`mermaid 代码块嵌入流程图、时序图等：
\`\`\`
\`\`\`mermaid
graph LR
  A[开始] --> B{判断}
...
**Bilibili（推荐，国内可直接播放）：**
\`<iframe src="https://player.bilibili.com/player.html?bvid=BVxxxxxx&high_quality=1" width="100%" allowfullscreen="true" scrolling="no"></iframe>\`

也可直接写视频页 URL，系统自动转换为嵌入地址：
\`<iframe src="https://www.bilibili.com/video/BVxxxxxx" width="100%" allowfullscreen="true" scrolling="no"></iframe>\`

**YouTube（缩略图模式，点击跳转播放）：**
\`[观看视频](https://www.youtube.com/watch?v=xxxxxxx)\`

**本地/直链视频：**
\`<video width="100%" controls><source src="视频地址.mp4" type="video/mp4"></video>\`
`,
  },
  apiDocs: {
    title: "API 文档 - 使用帮助",
...
### 访问方式
浏览器访问 \`/api-docs\`，即可查看完整的 Swagger UI 交互式 API 文档。

### 功能
- 自动从后端路由生成 OpenAPI 3.0 规范（50+ 端点）
- 按 10 个标签分组浏览
- 支持「Try it out」在线调试
- 自动携带登录 token（从 localStorage 读取）
- 无需额外工具即可测试 API

### 标签分组
认证 / 节点CRUD / Schema / 搜索与关联 / 仪表盘 / 导入导出 / 邮件 / AI / 邀请管理 / 系统管理
`,
  },
  auditLog: {
    title: "审计日志 - 使用帮助",
...
### 筛选方式
- **操作类型**：创建/更新/删除/进展/设置/升级/合并/点赞
- **实体类型**：节点/关系/表结构/设置/提案/提醒/wiki
...
| 操作人 | 执行操作的用户名 |
| 操作 | 创建/更新/删除/进展/设置/升级/合并/点赞（彩色 Tag）|
| 实体类型 | 节点/关系/表结构/设置/提案/提醒/wiki |
| 实体 | **显示语义化名称**（标题/姓名等），不再暴露内部 UUID；已删除的显示 \`(已删除)\` |
| 变更详情 | 具体变更字段：旧值 → 新值（标签形式）|

### 知识库审计
v2.9.0 起，知识库的所有写操作自动记录审计日志：
- **wiki.create** — 创建文章（记录标题、范围、是否加锁）
- **wiki.update** — 编辑文章（记录变更字段：标题/内容/加锁状态的前后值）
- **wiki.delete** — 删除文章（记录标题）
- **wiki.like / wiki.unlike** — 点赞/取消点赞（记录点赞数）
...
v2.3.4 起，Hermes 智能问答所用的 LLM 全部由本页面在 UI 内配置，无需登录服务器改 env 或重启后端。**保存即生效**，下一次 \`hermes:ask\` 调用就用新配置。
...
**2. /test env-fallback**
- v2.3.4 的「测试连接」按钮要求 DB 已有配置或 form 临时输入 apiKey 才能工作；v2.3.5 起新增 env fallback（\`HERMES_LLM_*\`）：admin 即使没存 DB，只要 backend 启动时设了 env 就能一键测试，方便首次配置引导
- 优先级链：body → DB → env → 默认
...
### 功能概述
知识图谱页面以力导向图的形式可视化展示系统中节点之间的关联关系。按节点类型着色（攻关单/人员/贡献等），支持筛选、搜索、展开/折叠、双击跳详情，是发现数据关联和排查问题的直观工具。

### 操作指南

**筛选节点类型**
- 页面顶部提供多选下拉，可勾选要展示的节点类型（攻关单、人员、贡献、团队贡献、信息卡片、版本包等）
- 取消勾选某类型后，该类型节点及其边会从画布中移除

**搜索节点**
- 在搜索框输入关键词，系统按名称/标题模糊匹配，匹配到的节点高亮居中

**单击节点 — 展开/折叠**
- 单击一个节点，系统获取该节点的 1 跳邻域（直接关联的节点和边）
- 再次单击同一个节点，折叠其独有的邻居（其他基图节点也在用的邻居不会移除）
- 折叠是级联的：如果 A 引入了 B，B 又引入了 C，折叠 A 时 B 和 C 都会移除（前提 C 没有被其他节点引用）

**双击节点 — 跳转详情**
- 双击任意节点，跳转到该节点的详情页（攻关单 → /attack/:id，人员 → /people，贡献 → /contributions）

**图例**
- 右下角图例按实际出现的节点类型动态生成，显示颜色和类型名称

**AI 问答浮窗**
- 右下角「AI 问答」浮窗按钮（与攻关详情的 AI 助手同一能力）
- 点击展开对话式问答，可提问关于当前图谱数据的问题
- 答案带可点击引用，点击后跳转到对应节点详情

### 实际场景示例
> 指挥者想了解「张三」参与的所有工作。在搜索框输入「张三」，图谱自动定位到张三节点并高亮。单击张三节点，展开其邻域：显示他参与的 5 个攻关单和 8 条贡献记录。双击其中一个攻关单节点跳转详情，查看具体进展。

### 注意事项
- 初次加载仅显示按类型筛选后的前 N 个节点，单击逐步展开邻域
- 图谱数据来源于结构化数据的边关系，不包含推导关系
- 大量节点时力导向布局需要几秒稳定，请耐心等待`,
  },
...
> **菜单位置**：系统管理 → 操作日志（**仅管理员可见可访问**）

### 功能概述
操作日志记录前端用户的 API 调用、页面导航和全局错误，是审计日志（后端写操作追踪）的补充视角。管理员可按会话、用户、类别、时间范围过滤查看，也可控制开关和清理历史数据。

### 日志类别

| 类别 | 说明 | 颜色 |
|------|------|------|
| API 调用 | 前端发起的所有后端 API 请求（排除操作日志自身的请求） | 蓝色 |
| 页面导航 | 路由切换记录（进入哪个页面、何时进入） | 青色 |
| 错误 | 浏览器全局未捕获错误 | 红色 |
| 用户操作 | 前端自定义操作事件 | 绿色 |
...
**查看日志**
- 页面加载后自动获取最近的操作日志条目
- 每条显示：时间、会话ID、用户名、类别、详细信息

**筛选**
- **类别**：下拉选择 API调用/页面导航/错误/用户操作
- **用户名**：输入用户名过滤
- **会话ID**：输入会话ID过滤（同一浏览器标签页的会话ID相同）
- **时间范围**：选择起止日期

**开关控制**
- 页面顶部有操作日志开关（Switch 组件）
- 关闭后前端不再记录新的操作日志（已记录的不受影响）
- 默认开启
...
> **菜单位置**：系统管理 → 系统升级（**仅管理员可见可访问**）

### 功能概述
系统升级页面支持通过上传升级包（.tar.gz）实现一键升级。升级过程全自动：备份 → 分析差异 → Schema 合并 → 代码替换 → 服务重启 → 健康检查。失败时自动回滚到升级前状态。

### 升级流程

**1. 上传升级包**
- 点击上传区域或将 .tar.gz 文件拖拽到上传区
- 系统自动分析包内容，展示版本差异

**2. 确认升级**
- 页面展示当前版本和目标版本的差异信息
- 需要在确认输入框中输入 \`UPGRADE\` 才能启动升级（防止误操作）

**3. 自动执行**
升级过程分以下阶段（实时日志展示进度）：
1. **备份**：备份当前数据库和代码
2. **解包**：解压升级包到临时目录
3. **Schema 合并**：三方合并用户自定义字段与新基线字段（冲突会报告）
4. **密钥迁移**：保留现有加密密钥和配置
5. **代码替换**：用新版本替换旧代码
6. **重启服务**：systemctl restart combat-v2
7. **健康检查**：验证服务启动成功
...
### Schema 三方合并
- **基线字段**：随升级包替换（新增/修改/删除）
- **用户自定义字段**（\`data/schemas-overlay/\`）：跨升级保留
- **冲突**：用户字段名与新基线字段名冲突时，生成冲突报告，保留用户版本

### 注意事项
- 升级期间服务会短暂中断（通常 < 30 秒）
- 确保升级包版本兼容（不能跨大版本降级）
- 升级前建议手动下载一份数据库备份
- 管理员需确保 systemd sudoers 已配置（\`/etc/sudoers.d/combat-v2\`）`,
  },
...
### 功能概述
共享功能提供统一的跨用户、跨攻关单的内容分享机制，覆盖知识库文章、攻关单、信息广场卡片三类实体。支持两种分享方式：生成外部链接（供未登录用户查看）和站内用户共享（推送给指定用户并通知）。

### 分享入口
- **知识库文章**: 文章右侧操作栏「分享」按钮（加锁文章不显示分享按钮）
- **攻关单详情**: 标题栏「分享」按钮
- **信息广场卡片**: 卡片详情「分享」按钮

### 创建分享（ShareModal 第一个 Tab）

#### 外部链接分享
1. 点击「分享」按钮，弹出 ShareModal
2. 选择「生成分享链接」
3. 可选配置：
   - **有效期**: 1 小时 / 1 天 / 7 天 / 30 天 / 永久
   - **访问密码**: 设置后访问者需输入密码才能查看
   - **最大查看次数**: 限制链接的访问上限
4. 点击「创建」→ 系统生成加密短链（如 \`http://xxx:3001/s/AbCdEfGh1234\`）
5. 点击「复制链接」分享给外部人员

#### 站内用户共享
1. 在 ShareModal 中选择「分享给站内用户」
2. 在用户搜索框中搜索并选择目标用户（支持多选）
3. 点击「确认分享」→ 目标用户在通知中心收到消息提醒

### 管理分享（ShareModal 第二个 Tab）

- **分享链接列表**: 显示当前实体的所有分享链接，包含 token、创建时间、访问次数、过期时间
- **访问趋势图**: 柱状图展示最近 30 天每日访问量
- **撤销分享**: 点击「撤销」按钮使链接失效，访问者将无法再查看

### 公共分享页面 (\`/s/:token\`)

- 无需登录即可访问
- 显示分享内容标题、正文（Markdown 渲染）、分享人、分享时间
- 如设置了访问密码，先弹出密码输入框
- 底部显示链接过期时间（如有）
- 攻关单分享仅展示脱敏内容（标题、状态、描述），不包含成员列表、私密授权、进展日志

### 安全规则
- **加锁文章**: 不允许生成分享链接
- **脱敏**: 攻关单分享不暴露敏感字段
- **Token 安全**: 12 字符 URL-safe token，不可预测、不可枚举
- **密码保护**: bcrypt 存储，timingSafeEqual 验证防时序攻击
- **审计**: 分享创建/撤销自动写入审计日志

### 实际场景示例
> 技术规范评审完成后，负责人在知识库打开「v2.0 接口规范」文章，点击「分享」→ 生成链接（有效期 7 天 + 访问密码 1234），复制链接发给外部合作方。合作方打开链接输入密码后查看规范内容。负责人在「管理」Tab 中看到该链接已被访问 15 次，评审结束后点击「撤销」使链接失效。`,
  },

  saasPlatform: {
    title: "SaaS 多租户 - 使用帮助",
    content: `## SaaS 多租户平台管理

### 功能概述
SaaS 多租户架构通过 \`SAAS_MODE=1\` 环境变量启用。启用后，平台运营方（superAdmin）可管理多个租户（团队/组织），每个租户数据完全隔离。不启用时系统仍为单租户模式，向后兼容。

### 三类角色

| 角色 | 范围 | 典型操作 |
|------|------|----------|
| superAdmin | 全平台 | 管理租户 CRUD、暂停/恢复租户、查看平台统计 |
| 租户 admin | 本租户 | 管理用户、配置中心、Schema、邮件、备份 |
| 租户 user | 本租户 | 攻关单 CRUD、人员、贡献、荣誉 |

### 登录页改造

登录页提供三种进入方式：

1. **登录**: 已有账号的用户输入用户名密码登录
2. **注册**: 新用户可选择「创建团队」（自动创建租户 + admin 账号）或「加入团队」（通过邀请码加入已有租户）
3. **免登录体验**: 自动创建 guest 用户（1 天有效期），无需注册即可体验系统
...
**租户列表**
- 展示所有租户：名称、标识(slug)、计划、状态、用户数、创建时间
- 操作：创建租户、编辑、暂停、恢复

**统计卡片**
- 总租户数、活跃租户数、总用户数、总节点数

### 租户详情页 (\`/platform/tenants/:id\`)
...
创建用户时系统自动检查当前租户的用户数是否超过计划配额。

### Guest 免登录体验

- 点击登录页「免登录体验」按钮，系统自动创建 guest 用户
- Guest 用户获得 1 天有效期的 JWT
- Guest 数据归入预创建的 \`guest\` 租户
- 系统每 24 小时自动清理 7 天前的 guest 数据

### 实际场景示例
> 平台运营方为三个团队创建租户：「云平台部」(pro)、「网络运维组」(free)、「安全团队」(enterprise)。每个团队的 admin 在注册页选择「创建团队」，输入团队名称获得 admin 账号。各团队数据完全隔离，互不可见。运营方在平台管理页面查看各团队的资源使用情况，发现「网络运维组」用户数接近 50 上限，提示该团队升级到 pro 计划。`,
  },
...
> **菜单位置**：系统管理 → 数据库迁移（**仅管理员可见可访问**）

### 功能概述
数据库迁移页面支持将数据从 SQLite 迁移到 PostgreSQL，适用于需要更高并发和可靠性的生产场景。页面展示当前数据库状态，并提供向导式迁移流程。
...
- 点击「开始迁移」按钮启动迁移
- 系统按表逐一复制数据，实时展示进度
- 迁移完成后自动切换到 PostgreSQL

### 迁移步骤
1. 填写目标 PostgreSQL 连接信息
2. 点击「测试连接」验证数据库可达
3. 点击「开始迁移」
4. 系统自动完成：
   - 在 PostgreSQL 中创建表结构
...
### 注意事项
- 迁移前请确保 PostgreSQL 服务正常运行且连接信息正确
- 迁移期间建议停止前端访问，避免数据不一致
- 首次迁移成功后，后续启动会自动使用 PostgreSQL（通过 \`DB_URL\` 环境变量）
- SQLite 数据库在迁移后不会被删除，可作为备份保留
- 大数据量迁移可能需要较长时间，请耐心等待进度完成`,
  },
  customCommands: {
    title: "自定义命令",
    content: `## 自定义命令

自定义命令是系统内置 CLI 的快捷模板。管理员可以预定义常用操作，用户一键执行。
...
### 三类关系
1. **升级链**：事件级别 → SLA 时长 → 上升角色（来自升级配置）
2. **人员分配**：每个角色负责的攻关单（来自实际的分配边）
3. **冲突关系**：攻关单之间的冲突以虚线表示

### 数据来源
- 升级规则：系统设置中的 escalation 配置
- 人员分配：知识图谱中的"分配"和"上报"边
- 冲突：知识图谱中的"冲突"边
