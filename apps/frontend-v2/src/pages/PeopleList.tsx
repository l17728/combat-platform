...
import { PlusOutlined, UploadOutlined, ExportOutlined, SearchOutlined, EditOutlined, UserOutlined } from '@ant-design/icons';
...
export default function PeopleList() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try { setNodes(await api.listNodes('person')); }
  ...
  const filtered = nodes.filter(n => {
    ...
    if (searchText) {
      ...
      return (p['姓名'] as string)?.toLowerCase().includes(s) ||
  ...
  const handleCreate = async (values: Record<string, unknown>) => {
    setSubmitting(true);
    try { await api.createNode('person', values); message.success('添加成功'); setDrawerOpen(false); form.resetFields(); fetchData(); }
  ...
  const handleEdit = async (values: Record<string, unknown>) => {
    ...
    try { await api.updateNode(editingNode.id, values); message.success('更新成功'); setEditOpen(false); setEditingNode(null); fetchData(); }
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url; a.download = `全员名单_${dayjs().format('YYYYMMDD')}.xlsx`; a.click();
  ...
  return (
    <div>
      ...
      <Space style={{ marginBottom: 16 }} wrap>
        <Select placeholder="部门筛选" allowClear style={{ width: 140 }} value={deptFilter} onChange={setDeptFilter}
          options={departments.map(d => ({ value: d, label: d }))} />
        <Input placeholder="搜索姓名/邮箱/工号" prefix={<SearchOutlined />} style={{ width: 240 }}
      ...
      <Drawer title="添加人员" width={480} open={drawerOpen} onClose={() => { setDrawerOpen(false); form.resetFields(); }} destroyOnClose maskClosable={false}
        extra={<Button type="primary" loading={submitting} onClick={() => form.submit()}>添加</Button>}>
        <Form form={form} layout="vertical" onFinish={handleCreate}>
          <Form.Item name="姓名" label="姓名" rules={[{ required: true, message: '请输入姓名' }]}><Input placeholder="姓名" /></Form.Item>
          <Form.Item name="工号" label="工号"><Input placeholder="工号" /></Form.Item>
          <Form.Item name="邮箱" label="邮箱"><Input placeholder="邮箱地址" /></Form.Item>
          <Divider orientation="left" orientationMargin={0}>组织信息</Divider>
          <Form.Item name="部门" label="部门"><Input placeholder="部门" /></Form.Item>
          <Form.Item name="角色" label="角色"><Input placeholder="角色" /></Form.Item>
        </Form>
      </Drawer>

      <Drawer title="编辑人员" width={480} open={editOpen} onClose={() => { setEditOpen(false); setEditingNode(null); }} destroyOnClose maskClosable={false}
      ...
      <Drawer title="人员详情" width={560} open={detailOpen} onClose={() => { setDetailOpen(false); setDetailNode(null); }} destroyOnClose
        extra={<Space>
        ...
        {detailNode && (
          <div>
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
              ...
              <Title level={4} style={{ margin: 0 }}>{String(detailNode.properties['姓名'] ?? '-')}</Title>
              <Text type="secondary">{String(detailNode.properties['部门'] ?? '')}{detailNode.properties['部门'] && detailNode.properties['角色'] ? ' · ' : ''}{String(detailNode.properties['角色'] ?? '')}</Text>
            </div>
      ...
      <Drawer title="导入全员名单" width={480} open={importOpen} onClose={() => setImportOpen(false)} destroyOnClose maskClosable={false}>
        <div style={{ marginBottom: 16 }}><Button onClick={handleExport} size="small">下载模板</Button></div>
        <Upload.Dragger accept=".xlsx,.xls" showUploadList={false} customRequest={({ file }) => handleImport(file as File)}>
