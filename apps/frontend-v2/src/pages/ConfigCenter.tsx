...
export default function ConfigCenter() {
  ...
  const entries: SettingEntry[] = Object.entries(settings)
    ...
    .map(([key, val]) => ({ key, ...val }))
    .filter(
      (e) => !searchText || e.key.toLowerCase().includes(searchText.toLowerCase()) || e.label?.includes(searchText)
    );
  ...
  const getImpactFields = useCallback(
    (key: string): { nodeType: string; nodeLabel: string; fieldLabel: string; fieldId: string }[] => {
  ...
  return (
    <div>
      ...
      <Modal
        title={`确认删除配置项「${deleteTarget?.key ?? ""}」`}
        open={!!deleteTarget}
            ...
            return (
              <>
                ...
                {impacts.length > 0 ? (
                  ...
                  <Alert type="info" showIcon style={{ marginBottom: 12 }} message="此配置未被任何 schema 字段引用" />
                )}
