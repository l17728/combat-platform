...
export default function KGGraph() {
  ...
  const expandNode = useCallback(async (graph: Graph, nodeId: string, nodeType: string) => {
    try {
      ...
      const newEdges = data.edges.filter(
        (e) => !existingEdgeIds.has(e.id) && idsRef.current.has(e.source) && idsRef.current.has(e.target)
      );
  ...
  const toggleNode = useCallback(
    (graph: Graph, nodeId: string, nodeType: string) => {
  ...
  const openDetail = useCallback(async (nodeId: string, nodeType: string) => {
    ...
    setDetailNode(null);
    try {
      setDetailNode(await api.getNode(nodeId));
    } catch (e) {
      handleApiError(e);
    } finally {
  ...
  const switchLayout = useCallback(async (graph: Graph, type: "d3-force" | "dagre" | "radial") => {
    ...
    const config: any =
      type === "dagre"
        ? { type: "dagre", rankdir: "TB", nodesep: 30, ranksep: 60 }
        : type === "radial"
          ? { type: "radial", unitRadius: 80, linkDistance: 100 }
  ...
  const locateInGraph = useCallback(
    async (graph: Graph) => {
      if (graph.destroyed) return;
      ...
      if (hit) {
        try {
          await graph.focusElement(hit.id);
          message.success(`定位到「${hit.data.label}」`);
        } catch {
          fetchAndSet(graph);
        }
      } else {
  ...
  const reload = () => {
    if (graphRef.current) fetchAndSet(graphRef.current);
  };
    ...
    <div>
      ...
      >
        ...
        <Space wrap>
          ...
          <Tooltip title="在当前图中定位关键词命中的节点(找不到则按筛选重载)">
            <Button icon={<AimOutlined />} onClick={() => graphRef.current && locateInGraph(graphRef.current)}>
              定位
            </Button>
          </Tooltip>
        ...
        <Space size={4} wrap>
          {presentEdgeTypes.length > 0 && (
            <Select
              ...
              onChange={(v) => {
                setEdgeTypeFilter(v);
                if (graphRef.current) applyEdgeFilter(graphRef.current, v);
              }}
          ...
          <Select
            ...
            onChange={(v) => {
              setLayoutType(v);
              if (graphRef.current) switchLayout(graphRef.current, v);
            }}
            style={{ width: 130 }}
      ...
      <Drawer
        title={
          detailNode ? (
            <Space>
              <Text strong>{nodeLabel(detailNode)}</Text>
              <Tag color={TYPE_COLORS[detailNodeType] ?? "default"}>
                {NODE_TYPE_LABEL[detailNodeType] ?? detailNodeType}
              </Tag>
            </Space>
          ) : (
            "节点详情"
            ...
            <Space>
              {expandedRef.current.has(detailNode.id) ? (
                <Button
                  size="small"
                  onClick={() => {
                    if (!graphRef.current || !detailNode) return;
                    collapseNode(graphRef.current, detailNode.id);
                    setDetailNode(null);
                  }}
                >
                  折叠关联
                ...
                <Button
                  ...
                  onClick={() => {
                    if (!graphRef.current || !detailNode) return;
                    expandNode(graphRef.current, detailNode.id, detailNodeType);
                    setDetailNode(null);
                  }}
                >
                  展开 1 跳
              ...
              <Button
                size="small"
                onClick={() => detailNode && navigate(detailPath({ nodeType: detailNodeType, id: detailNode.id }))}
              >
                跳详情页
              </Button>
            </Space>
      ...
      >
        ...
        {detailNode && (
          <Descriptions bordered size="small" column={1}>
            {Object.entries(detailNode.properties)
              .filter(([, v]) => v != null && v !== "")
              .map(([k, v]) => (
