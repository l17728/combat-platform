...
export default function DocumentCenter() {
  ...
  const copyMarkdown = async (d: DocItem) => {
    ...
    if (ok) message.success("Markdown 已复制");
    else message.warning("复制失败，请手动复制");
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "size",
      width: 90,
      render: (v: number | null, d: DocItem) => (d.type === "link" ? "-" : fmtSize(v)),
    },
    { title: "上传人", dataIndex: "uploadedBy", width: 100, render: (v: string | null) => v || "-" },
    {
      ...
      width: 150,
      render: (v: string) => (
        <Tooltip title={dayjs(v).format("YYYY-MM-DD HH:mm")}>{dayjs(v).format("MM-DD HH:mm")}</Tooltip>
      ),
    ...
    <div>
      ...
      <Upload.Dragger {...uploadProps} style={{ marginBottom: 16 }}>
        ...
        <p className="ant-upload-hint">
          支持多文件，单个最大 50MB。若点击上方按钮无法弹出系统选择框，请直接把文件从资源管理器拖到这里。
        </p>
      </Upload.Dragger>
      ...
      >
        <Form form={linkForm} layout="vertical" onFinish={handleAddLink} initialValues={{ name: "", url: "" }}>
          ...
          <Form.Item
            name="url"
            label="链接地址"
            rules={[{ required: true, message: "请输入链接", type: "url", warningOnly: false }]}
          >
