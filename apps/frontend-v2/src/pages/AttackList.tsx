...
export default function AttackList() {
  ...
  useEffect(() => {
    try {
      setFavorites(new Set(JSON.parse(localStorage.getItem(currentKey) || "[]")));
    } catch {
  ...
  const fieldOptions = useMemo(() => {
    if (!schema) return [];
    return schema.fields.filter((f) => !f.retired).map((f) => ({ value: f.name, label: f.label || f.name }));
  }, [schema]);
  ...
  const handleBatchDelete = async () => {
    ...
    setSelectedRowKeys([]);
    if (ok > 0)
      message.success(`已删除 ${ok} 条${fail ? ",失败 " + fail : ""}${skipped ? ",跳过非本人创建 " + skipped : ""}`);
  ...
  const columns = useMemo(() => {
    ...
    const fieldColMap: Record<string, Record<string, unknown>> = {
      标题: {
        ...
        sorter: (a: GraphNode, b: GraphNode) =>
          ((a.properties["标题"] as string) ?? "").localeCompare((b.properties["标题"] as string) ?? ""),
      },
      状态: {
        key: "状态",
        title: "状态",
        ...
        sorter: (a: GraphNode, b: GraphNode) =>
          ((a.properties["状态"] as string) ?? "").localeCompare((b.properties["状态"] as string) ?? ""),
      },
      当前处理人: {
        key: "处理人",
        title: "处理人",
    ...
    const dataCols = visibleColumns.filter((name) => fieldColMap[name]).map((name) => fieldColMap[name]);
      ...
      .map((name) => {
        ...
        return {
          ...
          render: (v: unknown) => {
            if (v == null) return "-";
            if (f?.type === "datetime")
              return <Tooltip title={String(v)}>{dayjs(String(v)).format("MM-DD HH:mm")}</Tooltip>;
    ...
    const opsCol = {
      ...
      render: (_: unknown, r: GraphNode) => {
        ...
        return (
          <Popconfirm
            title={`确认删除「${r.properties["标题"] ?? "(未命名)"}」？`}
    ...
    <div>
      ...
      <Space style={{ marginBottom: 16 }} wrap>
        ...
        {filterField && uniqueValues.length > 0 && (
          <Checkbox.Group
            value={filterValues}
            onChange={(v) => setFilterValues(v as string[])}
            ...
            <div style={{ maxHeight: 360, overflowY: "auto", minWidth: 200 }}>
              <div style={{ marginBottom: 8 }}>
                <Button
                  type="link"
                  size="small"
                  onClick={() => handleVisibleColumnsChange(columnOptions.map((o) => o.value))}
                >
                  全选
                </Button>
                <Button type="link" size="small" onClick={() => handleVisibleColumnsChange(DEFAULT_VISIBLE)}>
                  重置默认
                </Button>
              </div>
              <Checkbox.Group
      ...
      {selectedRowKeys.length > 0 && (
        ...
        >
          ...
          <Space>
            <Button size="small" icon={<StarOutlined />} onClick={handleBatchFavorite}>
              批量加关注
            </Button>
            <Popconfirm
          ...
          <Table
            ...
            onRow={(r) => ({
              onClick: (e) => {
                ...
                // 选择 checkbox 时不要触发跳转
                if (tag === "A" || tag === "INPUT" || (e.target as HTMLElement).closest(".ant-checkbox-wrapper"))
                  return;
              ...
              style: {
                cursor: "pointer",
                ...(favorites.has(r.id) ? { background: "#fffbe6", boxShadow: "inset 3px 0 0 #fadb14" } : {}),
              },
            })}
          />
        </FlexWrapper>
