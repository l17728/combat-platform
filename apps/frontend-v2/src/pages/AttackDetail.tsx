...
export default function AttackDetail() {
  ...
  const missingFields =
    schema?.fields.filter((f) => !f.retired && f.required && !node.properties[f.name]?.toString().trim()) ?? [];
  ...
  const handleSupportSubmit = async (values: any) => {
    ...
    try {
      if (editingNode) {
        await api.updateSupportNode(editingNode.id, values);
  ...
  const drColumns = [
    ...
    { title: "创建时间", dataIndex: "createdAt", width: 150, render: (v: string) => dayjs(v).format("MM/DD HH:mm") },
    {
      ...
      render: (_: unknown, r: DailyReportEntry) => (
        <Space size={4}>
          <Button size="small" type="link" onClick={() => setDrDetail(r)}>
            详情
          </Button>
          <Button
            size="small"
            type="link"
            disabled={r.status === "已发布"}
            onClick={() => {
              setEditingDr(r);
              drForm.setFieldsValue({ type: r.type, currentProgress: r.currentProgress, nextSteps: r.nextSteps });
              setDrModalOpen(true);
            }}
          >
            编辑
          ...
          <Button
            ...
            onClick={async () => {
              if (id) {
                await api.publishDailyReportEntry(id, r.id);
                message.success("已发布");
                fetchDailyReports();
              }
            }}
          ...
          <Button
            ...
            onClick={async () => {
              if (id) {
                await api.deleteDailyReportEntry(id, r.id);
                message.success("已删除");
                fetchDailyReports();
              }
            }}
  ...
  const memberColumns = [
    ...
    },
    {
      ...
      render: (_: unknown, _r: TeamMember, idx: number) => (
        <Space size={4}>
          <Button type="link" size="small" onClick={() => openEditMember(idx)}>
            修改角色
          </Button>
      ...
      children: (
        <div style={{ padding: "16px 0" }}>
          <Space style={{ marginBottom: 16 }}>
            <Button type="primary" icon={<PlusOutlined />} onClick={() => setProgressOpen(true)}>
              追加进展
            </Button>
            {isLeader && (
              <Button icon={<HistoryOutlined />} onClick={() => navigate(`/audit?entityId=${id}`)}>
                查看完整历史
              </Button>
            )}
          </Space>
      ...
      children: (
        <div style={{ padding: "16px 0" }}>
          ...
          <Table
            ...
            locale={{ emptyText: <Empty description="暂无日报条目" image={Empty.PRESENTED_IMAGE_SIMPLE} /> }}
          />
        </div>
      ),
    },
                    ...
                    <Space size={8}>
                      <Tooltip title={nd.note ? `备注：${nd.note}` : "无备注"}>
                        <Space
                          ...
                          onClick={() => selectSupportPerson(nd.personName)}
                        >
                          <Tag color="blue">{nd.category}</Tag>
                          <Text
                            strong
                            style={
                              selectedPersonName && nd.personName === selectedPersonName
                                ? { color: "#1677ff" }
                                : undefined
                            }
                          >
                        ...
                        return (
                          <List
                            ...
                            renderItem={(it, i) => {
                              ...
                              const nm = String(
                                p["标题"] ?? p["姓名"] ?? p["团队名称"] ?? p["name"] ?? nodeLabel(it.node)
                              );
                              return (
  ...
  const allTabItems = [...fixedTabItems.map((item) => ({ ...item, closable: false })), ...dynamicTabItems];
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
        ...
        <Space>
          ...
          {isCreator &&
            (isPrivate ? (
              <>
                <Button icon={<LockOutlined />} onClick={openPrivacyDrawer}>
                  管理私密授权
                </Button>
                ...
                extra={
                  <Space size={4}>
                    ...
                    <Button
                      ...
                      size="small"
                      icon={<CloseOutlined />}
                      onClick={() => setVisibleCards((prev) => prev.filter((k) => k !== "helpers"))}
                    />
                  </Space>
                }
              >
                <List
                  ...
                  renderItem={(h, i) => (
                    <List.Item style={{ padding: "6px 0" }}>
                      <Space>
                        <Tag color={i === 0 ? "gold" : i === 1 ? "#c0c0c0" : i === 2 ? "#cd7f32" : "default"}>
                          #{i + 1}
                        </Tag>
                        <Avatar size="small" icon={<UserOutlined />} />
                ...
                extra={
                  <Space size={4}>
                    ...
                    <Button
                      ...
                      size="small"
                      icon={<CloseOutlined />}
                      onClick={() => setVisibleCards((prev) => prev.filter((k) => k !== "audit"))}
                    />
                  </Space>
                }
              >
                ...
                <Button
                  ...
                  onClick={() => navigate(`/audit?entityId=${id}`)}
                  style={{ paddingLeft: 0 }}
                >
                  查看完整历史 →
                </Button>
      ...
      >
        <Form form={transForm} layout="vertical" onFinish={handleTransition}>
          ...
          <Button type="primary" htmlType="submit" loading={transSubmitting} block>
            确认流转
          </Button>
        </Form>
      </Drawer>
      ...
      <Drawer
        ...
        maskClosable={false}
        extra={
          <Button type="primary" onClick={() => privacyForm.submit()}>
            {isPrivate ? "保存" : "设为私密"}
          </Button>
        }
      >
      ...
      <Drawer
        ...
        maskClosable={false}
        extra={
          <Button type="primary" onClick={() => memberForm.submit()}>
            {editingMemberIdx == null ? "添加" : "保存"}
          </Button>
        }
      >
      ...
      >
        <Form form={progForm} layout="vertical" onFinish={handleAddProgress}>
          ...
          <Button type="primary" htmlType="submit" loading={progSubmitting} block>
            提交进展
          </Button>
        </Form>
      </Drawer>
      ...
      >
        <Form form={drForm} layout="vertical" initialValues={{ type: "进展通报" }} onFinish={createDailyReport}>
          <Form.Item name="type" label="日报类型">
            <Select options={DR_TYPES.map((t) => ({ value: t, label: t }))} />
          </Form.Item>
          ...
          <Form.Item style={{ marginBottom: 0, textAlign: "right" }}>
            <Space>
              ...
              <Button type="primary" htmlType="submit" loading={drSubmitting}>
                提交
              </Button>
            </Space>
          </Form.Item>
      ...
      >
        <Form form={supportForm} layout="vertical" onFinish={handleSupportSubmit}>
          ...
          <Form.Item name="category" label="大类" rules={[{ required: true, message: "请选择大类" }]}>
            <Select placeholder="选择大类" options={SUPPORT_CATEGORIES.map((c) => ({ value: c, label: c }))} />
          </Form.Item>
          ...
          <Form.Item name="status" label="状态" initialValue="待确认">
            <Select options={SUPPORT_STATUSES.map((s) => ({ value: s, label: s }))} />
          </Form.Item>
          <Form.Item name="note" label="备注">
          ...
          <Form.Item style={{ marginBottom: 0, textAlign: "right" }}>
            <Space>
              ...
              <Button type="primary" htmlType="submit" loading={supportSubmitting}>
                提交
              </Button>
            </Space>
          </Form.Item>
