...
test.describe("攻关单 - 成员管理与默认信息广场 tab", () => {
  ...
  test("新建攻关时 攻关成员 是多选,可选多人;创建后成员管理 tab 列表正确", async ({ page }, testInfo) => {
    ...
    const leaderItem = drawer
      .locator(".ant-form-item")
      .filter({ has: page.locator("label.ant-form-item-required, label").filter({ hasText: /^攻关组长$/ }) });
    await selectOption(page, leaderItem.locator(".ant-select").first(), "周组长");

    // 攻关成员 多选
    const memberItem = drawer
      .locator(".ant-form-item")
      .filter({ has: page.locator("label").filter({ hasText: /^攻关成员$/ }) });
  ...
  test("成员管理 tab 支持新增、修改角色、删除,且回写 攻关组长/攻关成员", async ({ page, request }) => {
    ...
    await selectOption(
      page,
      addDrawer.locator(".ant-form-item").filter({ hasText: "姓名" }).locator(".ant-select").first(),
      "周组长"
    );
    await selectOption(
      page,
      addDrawer.locator(".ant-form-item").filter({ hasText: "角色" }).locator(".ant-select").first(),
      "组长"
    );
    ...
    await selectOption(
      page,
      editDrawer.locator(".ant-form-item").filter({ hasText: "角色" }).locator(".ant-select").first(),
      "组员"
    );
