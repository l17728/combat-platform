  ...
  for (const e of await repo.queryEdges({ sourceId: nodeId, edgeType: MANUAL_EDGE })) {
    ...
    if (node)
      out.push({
        edgeId: e.id,
        direction: "out",
        sourceField: e.properties["sourceField"] ? String(e.properties["sourceField"]) : undefined,
        reason: String(e.properties["reason"] ?? ""),
        node,
      });
  }
...
export function makeRelationsRouter(repo: Repository): Router {
  ...
  r.post("/relations/manual", async (req, res) => {
    ...
    const s: GraphNode | null = await repo.getNode(sourceId),
      t: GraphNode | null = await repo.getNode(targetId);
