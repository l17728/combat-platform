...
async function writePinned(repo: Repository, pins: PinnedUi[], actor: string): Promise<void> {
  await repo.setSetting("ui_pinned", JSON.stringify(pins), actor);
}
...
export function makeUiCacheRouter(repo: Repository): Router {
  ...
  r.post("/ui-cache/pin", async (req, res) => {
    ...
    pins.unshift(pin);
    await writePinned(repo, pins.slice(0, 50), actor);
    log.info("ui.pin", { id: pin.id, label: pin.label, actor });
  ...
  r.patch("/ui-cache/pinned/:id", async (req, res) => {
    ...
    log.info("ui.pin.rename", { id: req.params.id, label: pin.label, actor });
    res.json(pin);
  });
  ...
  r.delete("/ui-cache/pinned/:id", async (req, res) => {
    ...
    await writePinned(repo, pins, actor);
    log.info("ui.unpin", { id: req.params.id, actor });
    res.json({ ok: true });
  });
