...
function triggerPostSaveJobs(repo: Repository, registry: SchemaRegistry): void {
  ...
  setImmediate(async () => {
    try { await syncConflicts(repo); } catch (e) { log.warn("post_save.conflicts.fail", { error: (e as Error).message }); }
    try { await scanEscalation(repo); } catch (e) { log.warn("post_save.escalation.fail", { error: (e as Error).message }); }
    try { await scanAndCreateReminders(repo, registry); } catch (e) { log.warn("post_save.reminders.fail", { error: (e as Error).message }); }
  });
}
...
export function makeRouter(repo: Repository, registry: SchemaRegistry): Router {
  ...
  r.patch("/schema/:nodeType", async (req, res) => {
    ...
    try {
      ...
      await repo.logAudit({ action: `SCHEMA_${req.body?.op}`, entityType: "schema",
