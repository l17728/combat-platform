...
// Row types: mirror SQLite table schemas (see db.ts). Keep field names identical to columns.
interface NodeRow {
  id: string;
  nodeType: string;
...
export class SqliteRepository implements Repository {
  ...
  logAudit(entry: { action: string; entityType: string; entityId: string; changes: unknown; actor: string }): void {
    this.db
      .prepare(`INSERT INTO audit_log VALUES (@id,@action,@entityType,@entityId,@changes,@by,@at)`)
      .run({
        id: randomUUID(),
        action: entry.action,
  ...
  createNode(nodeType: string, properties: Record<string, unknown>, actor: string): GraphNode {
    ...
    this.db.transaction(() => {
      this.db.prepare(`INSERT INTO nodes VALUES (@id,@nodeType,@properties,@search,@c,@u)`).run({
        id: node.id,
        nodeType,
        properties: JSON.stringify(properties),
        search: Object.values(properties)
  ...
  updateNode(id: string, patch: Record<string, unknown>, actor: string): GraphNode {
    ...
    const now = new Date().toISOString();
    this.db.transaction(() => {
      this.db.prepare(`UPDATE nodes SET properties=?, search_text=?, updated_at=? WHERE id=?`).run(
  ...
  queryNodes(nodeType: string, filter?: NodeFilter): GraphNode[] {
    const rows = this.db
      .prepare(`SELECT * FROM nodes WHERE nodeType=? ORDER BY created_at DESC`)
      .all(nodeType) as NodeRow[];
    let out: GraphNode[] = rows.map((r) => ({
      id: r.id,
      nodeType: r.nodeType,
      properties: JSON.parse(r.properties),
      createdAt: r.created_at,
  ...
  createEdge(
    ...
    this.db.transaction(() => {
      this.db
        .prepare(`INSERT INTO edges VALUES (@id,@edgeType,@s,@t,@p,@c,@u)`)
  ...
  deleteEdgeById(id: string, actor: string): boolean {
    const row = this.db.prepare(`SELECT id, edgeType, sourceId, targetId FROM edges WHERE id=?`).get(id) as
      | { id: string; edgeType: string; sourceId: string; targetId: string }
      | undefined;
    if (!row) return false;
    this.db.transaction(() => {
  ...
  appendProgress(ownerId: string, content: string, statusSnapshot: string, actor: string): ProgressLog {
    let p!: ProgressLog;
    this.db.transaction(() => {
      const max = this.db.prepare(`SELECT MAX(seqNo) m FROM progress_log WHERE ownerId=?`).get(ownerId) as
        | MaxSeqRow
        | undefined;
      p = {
        id: randomUUID(),
      ...
      this.db
        .prepare(`INSERT INTO progress_log VALUES (@id,@ownerId,@seqNo,@content,@s,@by,@at)`)
  ...
  listProgress(ownerId: string): ProgressLog[] {
    const rows = this.db
      .prepare(`SELECT * FROM progress_log WHERE ownerId=? ORDER BY seqNo`)
      .all(ownerId) as ProgressRow[];
    return rows.map((r) => ({
      id: r.id,
      ownerId: r.ownerId,
      seqNo: r.seqNo,
  ...
  listAllProgress(): ProgressLog[] {
    const rows = this.db.prepare(`SELECT * FROM progress_log ORDER BY ownerId, seqNo`).all() as ProgressRow[];
    return rows.map((r) => ({
      id: r.id,
      ownerId: r.ownerId,
      seqNo: r.seqNo,
  ...
  createProposal(
    ...
    this.db.transaction(() => {
      this.db
        .prepare(`INSERT INTO proposals VALUES (@id,@s,@t,@rt,@c,@ps,@r,@st,@db,@da,@ca)`)
        .run({
          id: row.id,
          s: row.sourceNodeId,
          t: row.targetNodeId,
  ...
  private mapProposal(r: ProposalRow): RelationProposal {
    return {
      id: r.id,
  ...
  listProposals(opts: { status?: RelationProposalStatus } = {}): RelationProposal[] {
    ...
    const rows = opts.status
      ? (this.db.prepare(`SELECT * FROM proposals WHERE status=?`).all(opts.status) as ProposalRow[])
      : (this.db.prepare(`SELECT * FROM proposals`).all() as ProposalRow[]);
    return rows.map((r) => this.mapProposal(r));
  }
  getProposal(id: string): RelationProposal | undefined {
    const r = this.db.prepare(`SELECT * FROM proposals WHERE id=?`).get(id) as ProposalRow | undefined;
  ...
  updateProposalStatus(id: string, status: RelationProposalStatus, decidedBy: string, actor: string): RelationProposal {
    ...
    this.db.transaction(() => {
      this.db
        .prepare(`UPDATE proposals SET status=?, decided_by=?, decided_at=? WHERE id=?`)
  ...
  createReminder(
    ...
    this.db.transaction(() => {
      this.db
        .prepare(`INSERT INTO notifications VALUES (@id,@k,@t,@rpid,@rn,@sub,@body,@st,@db,@da,@ca)`)
        .run({
          id: row.id,
          k: row.kind,
          t: row.ticketId,
          rpid: row.recipientPersonId ?? null,
          rn: row.recipientName,
          sub: row.subject,
          body: row.body,
          st: row.status,
          db: null,
  ...
  private mapReminder(r: ReminderRow): Reminder {
    return {
      id: r.id,
  ...
  listReminders(opts: { status?: ReminderStatus } = {}): Reminder[] {
    ...
    const rows = opts.status
      ? (this.db
          ...
          .all(opts.status) as ReminderRow[])
      : (this.db.prepare(`SELECT * FROM notifications ORDER BY created_at DESC`).all() as ReminderRow[]);
    return rows.map((r) => this.mapReminder(r));
  }
  getReminder(id: string): Reminder | undefined {
    const r = this.db.prepare(`SELECT * FROM notifications WHERE id=?`).get(id) as ReminderRow | undefined;
  ...
  updateReminderStatus(id: string, status: ReminderStatus, decidedBy: string, actor: string): Reminder {
    ...
    this.db.transaction(() => {
      this.db
        .prepare(`UPDATE notifications SET status=?, decided_by=?, decided_at=? WHERE id=?`)
        .run(status, decidedBy, at, id);
