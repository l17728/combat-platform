...
export class SqliteRepository implements Repository {
  ...
  private flatten(properties: Record<string, unknown>): string {
    return Object.values(properties).map(v =>
  ...
  async listAuditLog(filter: { action?: string; entityType?: string; entityId?: string; limit?: number }): Promise<AuditLogEntry[]> {
    ...
    if (filter.entityType) { where.push(`"entityType" = ?`); params.push(filter.entityType); }
    if (filter.entityId) { where.push(`"entityId" = ?`); params.push(filter.entityId); }
  ...
  async getSetting(key: string): Promise<string | null> {
    const row = await this.adapter.queryOne<{ value: string }>(
  ...
  async deleteEdges(opts: { sourceId?: string; targetId?: string; edgeType?: string }, actor: string): Promise<void> {
    ...
    await this.adapter.transaction(async (tx) => {
      for (const e of victims) {
        ...
        await this.auditTx(tx, "DELETE", "edge", e.id, { edgeType: e.edgeType, sourceId: e.sourceId, targetId: e.targetId }, actor);
      }
    });
  }
  ...
  async deleteEdgeById(id: string, actor: string): Promise<boolean> {
    const row = await this.adapter.queryOne<{ id: string; edgeType: string; sourceId: string; targetId: string }>(
      `SELECT id, "edgeType", "sourceId", "targetId" FROM edges WHERE id = ?`, [id],
    );
    if (!row) return false;
    await this.adapter.transaction(async (tx) => {
      await tx.run(`DELETE FROM edges WHERE id = ?`, [id]);
      await this.auditTx(tx, "DELETE", "edge", id, { edgeType: row.edgeType, sourceId: row.sourceId, targetId: row.targetId }, actor);
    });
    return true;
  }
  ...
  async appendProgress(ownerId: string, content: string, statusSnapshot: string, actor: string): Promise<ProgressLog> {
    ...
    await this.adapter.transaction(async (tx) => {
      const max = await tx.queryOne<{ m: number | null }>(
        `SELECT MAX("seqNo") as m FROM progress_log WHERE "ownerId" = ?`, [ownerId],
      );
  ...
  async listProgress(ownerId: string): Promise<ProgressLog[]> {
    const rows = await this.adapter.query<any>(
      `SELECT * FROM progress_log WHERE "ownerId" = ? ORDER BY "seqNo"`, [ownerId],
    );
  ...
  async createProposal(p: Omit<RelationProposal,"id"|"status"|"decidedBy"|"decidedAt"|"createdAt">, actor: string): Promise<RelationProposal> {
    ...
    await this.adapter.transaction(async (tx) => {
      await tx.run(
        ...
        [row.id, row.sourceNodeId, row.targetNodeId, row.relationType, row.confidence, row.proposerSource, row.rationale, row.status, null, null, now],
      );
  ...
  async createReminder(p: Omit<Reminder,"id"|"status"|"decidedBy"|"decidedAt"|"createdAt">, actor: string): Promise<Reminder> {
    ...
    await this.adapter.transaction(async (tx) => {
      await tx.run(
        ...
        [row.id, row.kind, row.ticketId, row.recipientPersonId ?? null, row.recipientName, row.subject, row.body, row.status, null, null, now],
      );
  ...
  async listReminders(opts: { status?: ReminderStatus } = {}): Promise<Reminder[]> {
    ...
    const rows = opts.status
      ? await this.adapter.query<any>(`SELECT * FROM notifications WHERE status = ? ORDER BY created_at DESC`, [opts.status])
  ...
  async updateReminderStatus(id: string, status: ReminderStatus, decidedBy: string, actor: string): Promise<Reminder> {
    ...
    await this.adapter.transaction(async (tx) => {
      await tx.run(
        `UPDATE notifications SET status = ?, decided_by = ?, decided_at = ? WHERE id = ?`,
        [status, decidedBy, at, id],
      );
