...
export async function runProposalScan(repo: Repository, registry: SchemaRegistry): Promise<number> {
  ...
  for (const pr of PROPOSERS)
    for (const d of await pr.propose(repo, registry)) {
      ...
      seen.add(k);
      await repo.createProposal(d, "scan");
      created++;
    }
...
export function makeProposalsRouter(repo: Repository, registry: SchemaRegistry): Router {
  ...
  r.post("/proposals/scan", async (_req, res) => {
    res.json({ created: await runProposalScan(repo, registry) });
  });

  r.get("/proposals", async (req, res) => {
    const status = req.query.status as string | undefined;
    res.json(await repo.listProposals(status ? { status: status as any } : {}));
  });
  ...
  r.post("/proposals/:id/decide", async (req, res) => {
    ...
    if (d === "拒绝" || d === "已拒绝")
      return res.json(await repo.updateProposalStatus(p.id, "已拒绝", decidedBy, decidedBy));
    if (d === "通过" || d === "已通过" || d === "修正") {
      const corrected = d === "修正" && patch?.targetNodeId;
      const target = corrected ? patch.targetNodeId : p.targetNodeId;
      if (corrected && !(await repo.getNode(patch.targetNodeId)))
      ...
      if (p.relationType === "SAME_AS") {
        ...
        await mergePerson(repo, p.sourceNodeId, target, decidedBy);
      }
      return res.json(await repo.updateProposalStatus(p.id, "已通过", decidedBy, decidedBy));
    }
