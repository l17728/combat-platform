...
export function makeDailyReportEntryRouter(adapter: DbAdapter): Router {
  ...
  r.put(
    "/nodes/:id/daily-reports/:eid",
    asyncHandler(async (req, res) => {
      const existing = await adapter.queryOne<any>(`SELECT * FROM daily_report_entry WHERE id=? AND ticket_id=?`, [
        req.params.eid,
        req.params.id,
      ]);
      ...
      await adapter.run(`UPDATE daily_report_entry SET type=?, current_progress=?, next_steps=? WHERE id=?`, [
        type ?? existing.type,
      ...
      const updated = await adapter.queryOne<any>(`SELECT * FROM daily_report_entry WHERE id=?`, [req.params.eid]);
  ...
  r.post(
    "/nodes/:id/daily-reports/:eid/publish",
    asyncHandler(async (req, res) => {
      const existing = await adapter.queryOne<any>(`SELECT * FROM daily_report_entry WHERE id=? AND ticket_id=?`, [
        req.params.eid,
        req.params.id,
      ...
      await adapter.run(`UPDATE daily_report_entry SET status='已发布', published_at=? WHERE id=?`, [
        publishedAt,
        req.params.eid,
      ]);
  ...
  r.delete(
    "/nodes/:id/daily-reports/:eid",
    asyncHandler(async (req, res) => {
      const result = await adapter.run(`DELETE FROM daily_report_entry WHERE id=? AND ticket_id=?`, [
        req.params.eid,
        req.params.id,
      ]);
