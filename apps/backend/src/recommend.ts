...
export async function recommendHelpers(
  ...
  for (const ae of await repo.queryEdges({ sourceId: T.id, edgeType: "ANCHORED_TO" })) {
    ...
    for (const back of await repo.queryEdges({ targetId: anchor.id, edgeType: "ANCHORED_TO" })) {
      ...
      if (s.nodeType === "attackTicket")
        for (const pid of await refPersons(repo, s.id, "当前处理人"))
        ...
        for (const pid of await refPersons(repo, s.id, "贡献人"))
  ...
  const resolved: HelperRecommendation[] = [];
  for (const [pid, e] of entries) {
    const person = await repo.getNode(pid);
    if (!person) continue;
...
export function makeRecommendRouter(repo: Repository): Router {
  const r = Router();
  r.get("/recommend/helpers/:id", async (req, res) => {
    ...
    res.json(await recommendHelpers(repo, node.id, limit));
  });
  return r;
}
