...
export const COMMANDS: CliCommand[] = [
  ...
  {
    name: "backup:offsite",
    summary: "异地备份（打包 combat.db + config/schemas + overlay → SFTP 至远端；--dry-run 仅打包不上传）",
    usage:
      "backup:offsite --host <h> --remote-dir <p> [--user root] [--port 22] [--key <path>] [--db <path>] [--schemas <dir>] [--overlay <dir>] [--dry-run]",
    build: (_pos, opts) => {
      const body: Record<string, unknown> = {};
      if (str(opts.host)) body.host = str(opts.host);
      if (str(opts.user)) body.user = str(opts.user);
      if (str(opts.port)) body.port = Number(str(opts.port));
      if (str(opts["remote-dir"])) body.remoteDir = str(opts["remote-dir"]);
      if (str(opts.key)) body.keyPath = str(opts.key);
      if (str(opts.db)) body.dbPath = str(opts.db);
      if (str(opts.schemas)) body.schemasDir = str(opts.schemas);
      if (str(opts.overlay)) body.overlayDir = str(opts.overlay);
      if (opts["dry-run"]) body.dryRun = true;
      return { method: "POST", path: "/api/backup/offsite", body };
    },
  },
