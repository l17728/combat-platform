...
describe("honor e2e", () => {
  it("creating a contribution with 关联攻关单 builds a CONTRIBUTED_TO edge to the ticket", async () => {
    ...
    const c = await request(app)
      ...
      .send({ 贡献人: "张三", 关联攻关单: "GK-1", 贡献类型: "实施", 贡献等级: "核心" });
  ...
  it("leaderboard is level-weighted, sorted desc, with per-level/type counts and period filter", async () => {
    ...
    await request(app)
      .post("/api/nodes/contribution")
      .send({ 贡献人: "张三", 贡献类型: "实施", 贡献等级: "核心", 周期: "2026-Q2" });
    await request(app)
      .post("/api/nodes/contribution")
      .send({ 贡献人: "张三", 贡献类型: "设计", 贡献等级: "普通", 周期: "2026-Q2" });
    await request(app)
      .post("/api/nodes/contribution")
      .send({ 贡献人: "李四", 贡献类型: "协调", 贡献等级: "关键", 周期: "2026-Q1" });
  ...
  it("person profile lists the person's contributions with linked attackTicketId", async () => {
    ...
    await request(app)
      .post("/api/nodes/contribution")
      .send({ 贡献人: "王五", 关联攻关单: "GK-9", 贡献类型: "发现", 贡献等级: "关键" });
