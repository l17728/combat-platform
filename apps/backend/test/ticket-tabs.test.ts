...
describe("ticket-tabs router", () => {
  ...
  it("POST rejects invalid tabType", async () => {
    const res = await request(app).post("/api/tickets/t1/tabs").send({ tabType: "other", title: "bad type" });
    expect(res.status).toBe(400);
  });

  it("POST rejects missing title", async () => {
    const res = await request(app).post("/api/tickets/t1/tabs").send({ tabType: "link" });
    expect(res.status).toBe(400);
  });

  it("POST rejects empty title", async () => {
    const res = await request(app).post("/api/tickets/t1/tabs").send({ tabType: "link", title: "  " });
    expect(res.status).toBe(400);
  });

  it("auto-increments tab_order", async () => {
    await request(app).post("/api/tickets/t1/tabs").send({ tabType: "link", title: "first" });
    await request(app).post("/api/tickets/t1/tabs").send({ tabType: "custom", title: "second" });
    const res = await request(app).get("/api/tickets/t1/tabs");
  ...
  it("GET returns tabs sorted by tabOrder", async () => {
    await request(app).post("/api/tickets/t1/tabs").send({ tabType: "link", title: "A" });
    await request(app).post("/api/tickets/t1/tabs").send({ tabType: "custom", title: "B" });
    const res = await request(app).get("/api/tickets/t1/tabs");
    expect(res.body.map((t: any) => t.title)).toEqual(["A", "B"]);
  });

  it("PATCH updates title", async () => {
    const created = await request(app).post("/api/tickets/t1/tabs").send({ tabType: "link", title: "old" });
    const tabId = created.body.id;
    const res = await request(app).patch(`/api/tickets/t1/tabs/${tabId}`).send({ title: "new" });
    expect(res.status).toBe(200);
  ...
  it("PATCH updates content", async () => {
    const created = await request(app).post("/api/tickets/t1/tabs").send({ tabType: "custom", title: "tab" });
    const tabId = created.body.id;
    const res = await request(app).patch(`/api/tickets/t1/tabs/${tabId}`).send({ content: "# updated" });
    expect(res.status).toBe(200);
    expect(res.body.content).toBe("# updated");
  });

  it("PATCH returns 404 for non-existent tab", async () => {
    const res = await request(app).patch("/api/tickets/t1/tabs/nonexistent").send({ title: "x" });
    expect(res.status).toBe(404);
  });

  it("PATCH rejects no fields", async () => {
    const created = await request(app).post("/api/tickets/t1/tabs").send({ tabType: "link", title: "tab" });
    const tabId = created.body.id;
    const res = await request(app).patch(`/api/tickets/t1/tabs/${tabId}`).send({});
    expect(res.status).toBe(400);
  });

  it("DELETE removes a tab", async () => {
    const created = await request(app).post("/api/tickets/t1/tabs").send({ tabType: "link", title: "to delete" });
    const tabId = created.body.id;
  ...
  it("PUT /tickets/:id/tabs/order reorders tabs", async () => {
    const a = await request(app).post("/api/tickets/t1/tabs").send({ tabType: "link", title: "A" });
    const b = await request(app).post("/api/tickets/t1/tabs").send({ tabType: "custom", title: "B" });
    const idB = b.body.id;
    const idA = a.body.id;
  ...
  it("PUT /order rejects non-array", async () => {
    const res = await request(app).put("/api/tickets/t1/tabs/order").send({ order: "not-array" });
    expect(res.status).toBe(400);
  });

  it("tabs are scoped to ticket — different tickets have independent tabs", async () => {
    await request(app).post("/api/tickets/t1/tabs").send({ tabType: "link", title: "T1 only" });
    await request(app).post("/api/tickets/t2/tabs").send({ tabType: "custom", title: "T2 only" });
  ...
  it("POST sets createdBy (api when no auth)", async () => {
    const res = await request(app).post("/api/tickets/t1/tabs").send({ tabType: "custom", title: "creator test" });
    expect(res.status).toBe(201);
    expect(res.body.createdBy).toBe("api");
  });

  it("PATCH updates updatedAt timestamp", async () => {
    const created = await request(app).post("/api/tickets/t1/tabs").send({ tabType: "custom", title: "time test" });
    const tabId = created.body.id;
  ...
  it("PATCH ignores empty title update", async () => {
    const created = await request(app).post("/api/tickets/t1/tabs").send({ tabType: "link", title: "original" });
    const tabId = created.body.id;
    const res = await request(app).patch(`/api/tickets/t1/tabs/${tabId}`).send({ title: "   ", content: "kept" });
    expect(res.status).toBe(200);
  ...
  it("DELETE logs include title", async () => {
    const created = await request(app).post("/api/tickets/t1/tabs").send({ tabType: "custom", title: "log test" });
    const tabId = created.body.id;
  ...
  it("deleting a node cascades to ticket_tabs", async () => {
    const nodeRes = await request(app).post("/api/nodes/attackTicket").send({ 标题: "Cascade test", 状态: "待响应" });
    const nodeId = nodeRes.body.id;

    await request(app).post(`/api/tickets/${nodeId}/tabs`).send({ tabType: "custom", title: "tab1" });
    await request(app).post(`/api/tickets/${nodeId}/tabs`).send({ tabType: "link", title: "tab2" });
