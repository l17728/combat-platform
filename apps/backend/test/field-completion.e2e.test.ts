...
function make() {
  const repo = new SqliteRepository(
    new SqliteAdapter(openDb(join(mkdtempSync(join(tmpdir(), "combat-fc-")), "t.sqlite")))
  );
  return {
...
describe("增量37 字段补全 + 人员 ref 化（§53）", () => {
  it("53.1 attackTicket 资源ID/租户ID 落库；导入 故障局点 命中 局点 alias", async () => {
    ...
    const t = (
      await request(app)
        ...
        .send({ 标题: "断连单", 状态: "进行中", 资源ID: "RES-1", 租户ID: "TEN-9" })
    ).body;
    ...
    const r = await request(app)
      ...
      .attach("file", await xlsx([{ 标题: "导入单", 状态: "进行中", 故障局点: "华东二" }]), "x.xlsx");
  ...
  it("53.2 攻关组长/攻关申请人 ref→person：建单自动建 person + REF 边，related 可见，rebuild 后存活", async () => {
    ...
    const t = (
      await request(app)
        .post("/api/nodes/attackTicket")
        .send({ 标题: "ref单", 状态: "进行中", 攻关组长: "王组长", 攻关申请人: "李申请" })
    ).body;
    // person 节点被自动创建
