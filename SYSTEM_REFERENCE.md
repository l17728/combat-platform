...
Role: 普通 | Leader | 管理员 | superadmin
PRIVILEGED_ROLES = ["Leader", "管理员"]
// superadmin: 平台管理员，独占 /api/platform/*，启动时由 ensureSuperAdmin() 自动提升默认 admin
// isGuest: JWT payload 布尔标记，游客用户为 true，受三层只读防护
```
...
> **⚠️ 重要：租户数据隔离尚未实现。** 以下描述当前实现状态。

### 7.1 已实现（外壳层）

| 功能             | 实现 | 说明                                                                         |
| ---------------- | ---- | ---------------------------------------------------------------------------- |
| tenants 表 + DDL | ✅   | 16 张表有 tenant_id 列（db.ts 自动迁移）                                     |
| 租户 CRUD        | ✅   | `/api/platform/*` 仅 superadmin                                              |
| 注册 + 创建团队  | ✅   | `POST /api/auth/register {tenantName, tenantSlug}` → 自动创建 tenant + admin |
| 注册 + 邀请码    | ⚠️   | 邀请码表无 tenant_id 归属                                                    |
| SuperAdmin 角色  | ✅   | `ensureSuperAdmin()` 启动时自动提升默认 admin                                |
| Guest 只读防护   | ✅   | 三层：后端 guestReadOnlyMiddleware + 前端 req() guard + useGuestGuard        |
| 计划配额         | ✅   | free(50u/5kn) / pro(200u/50kn) / enterprise(unlimited)                       |
| Guest 数据清理   | ✅   | `cleanGuestData()` cron 清理 7 天前 guest 数据                               |

### 7.2 未实现（核心层）— ⚠️ 数据隔离缺失

**当前所有业务查询无 `WHERE tenant_id = ?` 过滤。** `repository.ts` 的 `queryNodes`、`createNode`、`queryAudit` 等核心方法不感知租户。`tenantMiddleware` 设置了 `req.tenantId` 但下游无消费者。

影响：任何租户的用户可以看到、操作所有租户的数据。

### 7.3 角色体系

| 角色                 | 范围               | 说明                                                  |
| -------------------- | ------------------ | ----------------------------------------------------- |
| superadmin           | 全平台             | 独占 `/api/platform/*`，`ensureSuperAdmin()` 自动提升 |
| admin                | 本租户（设计意图） | 当前实际可跨租户操作（隔离未实现）                    |
| leader               | 本租户             | ticket-tabs / documents 写权限                        |
| normal               | 本租户             | 基础读写                                              |
| guest (isGuest=true) | guest 租户         | 只读参观，三层防护                                    |

---

## 8. 共享类型关键契约（@combat/shared）
...
Role = "普通" | "Leader" | "管理员" | "superadmin"
PRIVILEGED_ROLES = ["Leader", "管理员"]
// JwtPayload 扩展: { userId, username, role, tenantId, isGuest? }
