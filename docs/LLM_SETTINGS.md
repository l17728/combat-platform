...
## v2.7 增量 — 模型列表动态刷新 + 部署 systemd drop-in 自动清理

### 模型列表动态刷新

v2.6 的「LLM 设置」页面里 `defaultModel` / `smallModel` 是 AutoComplete + 硬编码 `PROVIDER_DEFAULTS.models`(7-8 个常用 model 名)。问题:provider 改了/上新模型,前端不刷新就看不到。

v2.7 新增 `GET /api/llm-settings/models`(admin only):
...
`apps/backend/src/app.ts` 三层 fallback 链(DB → env → 默认)最末端的默认 model 由 v2.6 的 `glm-4.5-air` 改为 `glm-4-flash`。原因:v2.6 教训智谱 `zhipu` apiKey 在 OpenAI 兼容 endpoint 下 `glm-4.5/4.6/5` 都需余额,**`glm-4-flash` 免费可用**,作为零成本默认值更稳。
...
`POST /api/llm-settings/test` 的凭据优先级链由 v2.6 的 `body → DB` 扩展为 `body → DB → env`:
...
v2.7 起,`scripts/deploy-v2/deploy-direct.mjs` 在每次部署的 5/5 阶段(daemon-reload 之前)自动:
