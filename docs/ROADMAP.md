...
| v2.3.1     | 2026-05-31     | harden + resilience + upgrade-real             | exceljs/PM2/dist/backup/Sentry + KG outbox/Merkle/catch 迁移 + GitHub Release/PGP/prod-rehearsal                               |
| v2.3.2     | 2026-05-31     | Hot-fix                                        | React #310 hooks 顺序 + HermesChat 抖动                                                                                        |
| v2.3.3     | 2026-05-31     | Hermes Tool-using Agent                        | 14 工具 + filter DSL + 私单收口 + OpenAI tool-calling + golden set 15/15(工具直调)                                             |
| v2.3.4     | 2026-05-31     | LLM 端到端 + Inbox + 面包屑 + Schema-as-UI     | OpenAICompatibleRunner 直连 + LLM 设置 UI + 通知 Inbox/SSE + BreadcrumbBar + attackTicket schema 驱动                          |
| v2.3.5     | 2026-06-01     | Hermes 体验收尾 + Schema-as-UI 全栈化 + 多视图 | /models 动态 + /test env-fallback + Q7 prompt + drop-in 自愈 + 7 nodeType schema 驱动 + virtual schema + Kanban/Calendar/Pivot |
| v2.3.6     | 2026-06-01     | Hermes 高级能力                                | 写工具(create/update/progress) + 会话记忆 + golden set 20/20 + 双 DB 支持                                                      |
| v2.3.8     | 2026-06-01     | UX 中期 II                                     | 暗黑模式 + 产品 Tour(Dashboard/AttackList/Admin) + Dashboard 看板配置                                                          |
| **v2.3.9** | **2026-06-01** | **Webhook + 邮件摘要 + 内联字段**              | **Webhook 事件订阅(11种事件) + 邮件Digest(日/周汇总) + 攻关详情内联添加字段**                                                  |
| **v2.2.0** | **2026-06-01** | **邮件增强 + 邀请管理 + 运营大屏**             | **HTML邮件模板 + 自定义时间段摘要 + 邀请码注册(角色预设) + 深色全屏运营大屏(KPI/状态分布/自动刷新)**                           |

> v2.3.7(多租户 + 行级权限)跳过,相关需求合并到 v3.0。
...
| 10  | 审计 Merkle 根公示          | v3.2   | v2.3.1 已实现 Merkle 链         |
...
| 维度     | v2.0 baseline | v2.3.5 当前 | 目标 v3.0 |
| -------- | ------------- | ----------- | --------- |
...
- ✅ helmet/CSRF/rate-limit/AES 加密(v2.2/v2.3.4)
- ✅ KG outbox + worker 可重放(v2.3.1)
- ✅ audit Merkle 链(v2.3.1)
- ✅ Sentry 错误聚合(v2.3.1)
- ✅ 一键升级 UI + Schema overlay(v2.3)
- ✅ Hermes Tool-using Agent + 14 工具(v2.3.3)
- ✅ LLM UI 配置 + AES apiKey(v2.3.4)
- ✅ 站内 Inbox + SSE(v2.3.4)
- ✅ BreadcrumbBar(v2.3.4)
- ✅ Schema-as-UI 全栈(v2.3.4 attackTicket + v2.3.5 7 nodeType)
- ✅ 多视图 Kanban/Calendar/Pivot(v2.3.5)
- ✅ deploy-direct.mjs drop-in 自愈(v2.3.5)
