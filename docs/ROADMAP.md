...
| 版本 | 主题                  | 主要内容                                                                                          |
| ---- | --------------------- | ------------------------------------------------------------------------------------------------- |
| v3.0 | **多租户 + 行级权限** | 真多租户隔离(SaaS 化)、组织/团队/项目维度行级权限、租户 quota/配额、邀请/加入流程                 |
...
| v3.3 | **集成生态**          | IM 集成(钉钉/企业微信/飞书)、移动端 PWA 适配、邮件 digest 增强(HTML 模板+附件)                    |
| v3.4 | **业务延展**          | 工作流引擎(BPMN-light)、自定义报表设计器、知识库/Wiki、API 自动文档(OpenAPI 渲染)                 |
| v3.5 | **架构里程碑**        | GraphQL / 微服务拆分 / 完全 Postgres-only / KG 引擎独立 / 大屏运营仪表板 / i18n / a11y            |
...
| 4   | 架构 | 邀请/加入流程               | v3.0     | 邮件邀请链接 + 角色分配            |
| 5   | 性能 | Postgres 多副本部署         | v3.1     | 读写分离、连接池优化               |
| 6   | 性能 | 全链路 trace(OpenTelemetry) | v3.1     | HTTP → DB → LLM 全链路可观测       |
| 7   | 性能 | CDN/edge 缓存               | v3.1     | 静态资源 + 配置中心响应缓存        |
| 8   | 性能 | 前端 code-splitting         | v3.1     | 路由级懒加载，首屏 bundle 减重     |
| 9   | 性能 | Bundle 减重                 | v3.1     | Tree-shaking + 依赖审计            |
| 10  | 安全 | 完整渗透测试(10 用例真跑)   | v3.2     | OWASP Top 10 全覆盖                |
| 11  | 安全 | MFA 多因素认证              | v3.2     | TOTP/短信二次验证                  |
| 12  | 安全 | 审计 Merkle 根公示          | v3.2     | 防篡改验证接口                     |
| 13  | 安全 | 密码策略+锁定               | v3.2     | 复杂度规则 + 连续失败锁定          |
| 14  | 安全 | WAF                         | v3.2     | SQL 注入/XSS/CC 攻击防护           |
| 15  | 集成 | IM 集成(钉钉/企业微信/飞书) | v3.3     | 消息推送 + 审批回调                |
| 16  | 集成 | 移动端 PWA 适配             | v3.3     | Service Worker + 离线 + 安装到桌面 |
| 17  | 集成 | 邮件 digest 增强            | v3.3     | HTML 模板 + 附件 + 自定义时间段    |
| 18  | 业务 | 工作流引擎(BPMN-light)      | v3.4     | 可视化审批流 + 条件分支            |
| 19  | 业务 | 自定义报表设计器            | v3.4     | 拖拽式图表 + 导出 PDF              |
| 20  | 业务 | 知识库/Wiki                 | v3.4     | Markdown 编辑 + 目录树 + 全文搜索  |
| 21  | 业务 | API 自动文档(OpenAPI 渲染)  | v3.4     | Swagger UI 从 routes 自动生成      |
| 22  | UX   | i18n 国际化(中英切换)       | v3.5     | react-intl 全站文案外置            |
| 23  | UX   | a11y 可访问性               | v3.5     | 键盘导航 + ARIA 标签 + 对比度      |
| 24  | UX   | 大屏运营仪表板              | v3.5     | 实时数据可视化 + 投屏模式          |
