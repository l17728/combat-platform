...
## v2.2 P1 已实施(2026-05-31, 分支 `feature/roadmap-perf-p1`)

P1 全部 7 项落地。攻克"算法/架构"类热点 + Prometheus metrics 给可观测性补齐。

| #   | 项                                                                          | commit    | 评审条目           |
| --- | --------------------------------------------------------------------------- | --------- | ------------------ |
| 1   | `Repository.queryNodesByProperty` SQL 下推 + 9 个热点 key SQLite 表达式索引 | `cc9d915` | §1(主条目),配合 §2 |
| 2   | 5 callsite 改用下推(emailGroup.组名/person.邮箱/PB/anchor.key/import 主键)  | `8b22423` | §1 callsite 减负   |
| 3   | `syncConflictsForOne` 增量算法 + post-save 防抖累计 ticketId 集             | `6548942` | §2 全量重建        |
| 4   | `recommend.recommendHelpers` 消除 N+1                                       | `b69bbac` | §3                 |
| 5   | `proposer.HeuristicRelationProposer` levenshtein 长度差预筛                 | `28bc9a9` | §4                 |
| 6   | `appendProgress` 原子 seqNo(INSERT...SELECT COALESCE(MAX+1))                | `93fd37a` | §6                 |
| 7   | Prometheus metrics `/api/metrics`(无 auth)+ `combat_*` 指标族               | `d383a89` | 弱项 ✗ 2           |

### EXPLAIN 实测(SQLite, 10k seeded rows + ANALYZE)

```
EXPLAIN QUERY PLAN — queryNodesByProperty equivalent:
  SEARCH nodes USING INDEX idx_nodes_prop_status (nodeType=? AND <expr>=?)
...
### Benchmark(`scripts/bench-queryNodes.mjs`)

| N (attackTicket) | queryNodes(filter) median | queryNodesByProperty median | speedup |
| ---------------- | ------------------------- | --------------------------- | ------- |
...
speedup 在 1k-10k 区间稳定 4-6×。N 越大,JSON.parse 全部行的成本越高,优势越明显。

### 验证

- baseline 463 backend tests → 477 全绿(+5 queryNodesByProperty + 5 conflicts-incremental + 4 metrics,无回归)
- `npx tsc --noEmit` 双端均通过
- 7 个独立 commit,每个 commit 立刻 push,可滚动回退
- 改动总行数约 350 行,集中在 8 个 src 文件 + 3 个 test 文件
- 新增 1 个 npm 依赖:`prom-client@15`(零 native binding,纯 JS)
- 新增文档:`docs/PERFORMANCE_TUNING.md`

### 未做(留 P2 / P3)

- §5 Hermes fallback 全文搜索 — 需迁 PG + `to_tsvector + GIN`,本桶 SQLite-only,留迁移期一起做
- 后台 worker 进程分离 — 架构改造,P3
- 容器化 + K8s — 架构改造,P3
- rate-limit / circuit-breaker — 需 `express-rate-limit`,P1 下次桶
