...
## v2.2 P1 已实施 (2026-05-31, feature/roadmap-quality-p1)

四项重构合并入 master 前已落地,均在 `feature/roadmap-quality-p1` worktree 完成,backend 463/463 + frontend vitest 54/54 全绿。

### ① AttackDetail.tsx 1823 → 327 行 (-82%)

老巨石(`apps/frontend-v2/src/pages/AttackDetail.tsx`)拆为「布局壳 + 6 子组件 + 2 hook + 1 builder」:

```
pages/AttackDetail.tsx           327  // 状态声明 + props 转发,纯 layout
pages/attackDetail/
  AttackDetailHeader.tsx         265  // 标题 + 操作按钮 + Steps + 摘要卡 + 缺字段 Alert
  AttackBasicInfoTab.tsx          77  // 基础信息 tab (字段隐藏/恢复)
  AttackMembersTab.tsx            88  // 成员管理 tab
  AttackProgressTimelineTab.tsx   89  // 进展 Timeline (progress + 关键审计)
  AttackDailyReportTab.tsx       110  // 日报 Table + 操作列
  AttackSupportNetworkTab.tsx    244  // 求助网络 Tree + 负责人详情面板
  AttackDetailSidebar.tsx        127  // 找帮手 + 合规追溯 卡片
  AttackDetailDrawers.tsx        529  // 5 个 Drawer + 3 个 Modal 集中
  buildTabItems.tsx              250  // tab 配置构建
  useAttackDetailData.ts         139  // 数据加载 hook (fetchData/fetchDailyReports/...)
  useAttackDetailHandlers.ts     348  // 业务交互 hook (handleEdit/handleTransition/...)
```

页面文件不再持有任何业务逻辑,只把 state 灌给 hooks/子组件。新加 tab/字段类型不再要求理解整个文件。

### ② 前端 catch 统一 ApiError + 401 跳登录 + handleApiError

- `api.ts` 新增 `class ApiError extends Error { status, detail, path }` 和 `onUnauthorized(handler)` 钩子。所有 `req<T>` 失败抛 ApiError,401(除 `/auth/me` 探活)自动触发回调。
- `main.tsx` 注册全局 401 → 清 token + 跳 `/login`(避免登录页自循环)。
- 新增 `utils/handleApiError.ts`:401 静默 / 403 优先 detail / 非 ApiError 用 defaultMsg。
- 渐进迁移 9 处高频 catch(LoginPage / BugReport×4 / FloatingFeedback / AttackList×3)。其它 70+ 处保留旧 `message.error(e.message)` 兼容。
- 新增 `apiError.test.ts` (5 tests) + `handleApiError.test.ts` (6 tests)。

### ③ test/helpers.ts 单源,去 3 个本地副本

- `apps/backend/test/helpers.ts` 暴露 `makeRealSchemaTestApp(opts?)` + `REAL_SCHEMAS_DIR` 常量。
- merge/rbac/automation e2e 测试删除本地 makeApp/make,统一 import 单源。
- `opts.noAuth` 明确切换 `COMBAT_NO_AUTH`;rbac 测试默认清掉,防止 worker 间污染。

### ④ 前端 vitest 单测落地(起手 5+2 = 7 文件,54 tests)

新增 `apps/frontend-v2/vitest.config.ts` (jsdom + setupFiles) + `src/__tests__/`:

- `utils/teamMembers.test.ts` (13) — parseMembers / syncMemberFields / buildMembersFromForm
- `utils/auditFilter.test.ts` (9) — categorizeAudit / filterKeyAudits
- `utils/nodeLabel.test.ts` (10) — 字段优先级 / 兜底类型名 / 路由
- `utils/handleApiError.test.ts` (6) — 401 静默 + 文案兜底
- `hooks/useSettings.test.tsx` (5) — 缓存 + inflight dedupe + refresh
- `components/StatusTag.test.tsx` (6) — type 切换 + 颜色映射 + 图标
- `api/apiError.test.ts` (5) — ApiError 携带元信息 + onUnauthorized 钩子

根 `package.json` 加 `test:frontend:unit`,`test:all` 串入。CI workflow 新增 step。
