...
- **团队贡献（teamContribution）**：除个人贡献外，独立记录团队级贡献（团队名称、组长、组员、贡献等级、关联攻关单、周期）。贡献录入页同时呈现「个人贡献」「团队贡献」两张表；荣誉殿堂新增「团队荣誉」tab，按贡献等级分组展示团队卡片 + 详情。
...
| 作战 | 荣誉殿堂 | `/honor` | **P0-②** 个人排行（贡献加权榜单/个人档案）+ 团队荣誉（团队贡献按等级分组卡片） |
| 作战 | 贡献录入 | `/contributions` | 个人贡献 + 团队贡献两张表，分别录入 |
| 全局 | 作战态势/信息广场 | `/`（Dashboard Tabs） | 大盘统计 + 信息广场（infoCard 卡片发布，按重要程度/信息分类） |
